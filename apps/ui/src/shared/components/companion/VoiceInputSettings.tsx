...
/** The C0r row's action labels (frozen so tests and copy stay in lock-step). */
export const MODEL_AUDIO_USE_LOCAL_LABEL = 'Use local transcription';
...
/**
 * The C0r row's `data-state` brush. The row's anatomy mirrors the shipped model
 * row (`var(--hover-bg)` / `var(--border-color)`), with success/warning/error
 * token tints for the resolved states.
 */
...
/**
 * The demoted technical qualifier for the C0r row: the backend's `detail`,
 * shown only for the non-ready states (a raw string is NEVER the primary
 * sentence). Pure.
 */
...
/** The C0r caption colour per state (text carries the state; never colour alone). */
...
export const VoiceInputSettings: React.FC<VoiceInputSettingsProps> = ({
  ...
  // ── C0r — Model audio status (#2897 ST-6 / REQ-7) ──────────────────────────
  // The backend-owned capability probe runs ONLY while `Model audio` is
  // selected; the row consumes its typed verdict and never infers capability
  // from a model name.
  const {
    capability: modelAudioCapability,
    checking: modelAudioChecking,
    refresh: refreshModelAudioCapability,
  } = useModelAudioCapability(voiceHandling === 'model');
  ...
  // ── C0r — Model audio status actions (#2897 ST-6 / REQ-7) ──────────────────
  // `Use local transcription` is the user-initiated fallback: ONE click flips the
  // persisted setting (never an automatic silent transcription) and announces it.
  const handleUseLocalTranscription = useCallback(() => {
    setVoiceHandling(DEFAULT_VOICE_HANDLING);
    setAnnouncement('Speech handling set to Local transcription.');
  }, [setVoiceHandling]);
  ...
  return (
    <Box>
      ...
      <VStack align="stretch" gap={2}>
        ...
        {/* ── C0r — Model audio status (#2897 ST-6 / REQ-7) ──────────────── */}
        {/* Rendered while `Model audio` is selected. The capability is
            BACKEND-owned; the row offers the explicit one-click switch to local
            transcription (never an automatic silent fallback). */}
        {voiceHandling === 'model' && (
          <Box
                ...
                {(modelAudioRow.offerLocal ||
                  ...
                  <HStack gap={1} align="center" flexShrink={0}>
                    {modelAudioRow.offerLocal && (
                      <Button
                        size="sm"
                        data-testid="companion-voice-model-audio-use-local"
                        bg="var(--accent-primary)"
                        color="var(--accent-contrast)"
                        onClick={handleUseLocalTranscription}
                        _hover={{ opacity: 0.9 }}
                      >
                        {MODEL_AUDIO_USE_LOCAL_LABEL}
                      </Button>
                    )}
                    {modelAudioRow.offerChangeModel && (
                      <Button
