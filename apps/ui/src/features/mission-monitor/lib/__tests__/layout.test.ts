/**
 * Tests for the deterministic Mission Monitor graph geometry.
 *
 * #2835 sub-task 3: the force-directed layout tests were removed with the
 * production-dead `computeForceLayout` (the chain layout is the only mode —
 * #2760; the force pass was always overwritten by the deterministic geometry).
 * Kept: deterministic chain + SubagentNode companion-column geometry, the
 * residue rectangular de-overlap, and the companion-extent chain pitch.
 */
...
import type { RectNode, ChainSubagentNode } from '../layout';
