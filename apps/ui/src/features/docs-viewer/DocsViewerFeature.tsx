...
export class DocsViewerFeature extends FredoFeatureClass {
  ...
  processEvent(event: FredoEvent): void {
    ...
    if (event.state === 'Init') {
      ...
      this.state = {
        query: (input?.query as string | undefined) ?? (input?.keyword as string | undefined) ?? (input?.search as string | undefined) ?? null,
        results: [],
        source,
        timestamp,
      };
    ...
    if (event.state === 'Response' && payload) {
      // Normalise various response shapes into a flat results array
      const raw = (payload as any)?.results ?? (payload as any)?.items ?? (payload as any)?.value ?? payload;
