//! OpenCodeAdapter — transforms OpenCode hook events into FredoEvents.
//!
...
/// OpenCodeAdapter transforms OpenCode plugin hook events into FredoEvents.
///
/// REQ-2.1: Implements CommAdapter for the IPC hook transport
...
///
/// Spec #2449 S4: the OTLP half is removed (superseded by `GenericOtlpAdapter`);
/// OTLP transports return an empty vec here.
#[derive(Debug)]
...
impl CommAdapter for OpenCodeAdapter {
    ...
    async fn transform(
        ...
        match transport {
            ...
            // OTLP delivery is handled by GenericOtlpAdapter (Spec #2449 S2) —
            // OpenCodeAdapter no longer transforms OTLP spans (Spec #2449 S4).
            _ => Ok(vec![]),
        }
    }
}
...
mod tests {
    ...
    fn adapter_fields_count() {
        ...
        // OpenCodeAdapter has 4 Arc<Mutex<HashMap>> fields after Spec #2449 S4
        // removed the OTLP-only machinery (trace_to_session,
        // pending_task_instructions, parent_prompts):
        // session_to_correlation, tool_call_id, session_to_parent, session_turn_counter
        let adapter = OpenCodeAdapter::new();
        ...
        assert_eq!(
            ...
            "OpenCodeAdapter should have exactly 4 fields after the OTLP-half removal (Spec #2449 S4)"
        );
    }
