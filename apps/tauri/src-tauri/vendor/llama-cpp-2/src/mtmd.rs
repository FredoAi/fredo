...
impl From<&MtmdContextParams> for llama_cpp_sys_2::mtmd_context_params {
    fn from(params: &MtmdContextParams) -> Self {
        ...
        // Disable warmup inference: the warmup runs a full GGML compute graph
        // on a dummy image during init, which crashes on Gemma4V due to an
        // assertion in the RoPE/position-embedding path. Buffers are allocated
        // lazily on the first real inference call instead.
