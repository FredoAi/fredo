...
| 2026-06-30 | #369 | 4 capsules (PRs #375-#378), 0 bugs. Fixed 6 Mission Monitor data pipeline bugs + force-directed layout. | Four capsules: ECE init-before-end lifecycle fix (engine.rs +7), OTLP adapter payload mapping (opencode.rs +851/-102), FeatureStore idempotent insert (feature_store.rs +99), frontend graph building + d3-force layout (useMissionMonitor.ts +164, layout.ts +129 new). Key learnings: (1) ContractEngine skipped init when completeWhen fired on first event — init must emit before end. (2) OTLP spans deliver flat attribute maps, not nested Hook-style payloads — adapter must map gen_ai.usage.* and gen_ai.response.body into {info, part} shape. (3) FeatureStore INSERT OR IGNORE needed for idempotent delivery inserts. (4) Non-idempotent rendering on update deliveries wipes init-time content — spread-merge required. (5) d3-force integration for ReactFlow graphs with forceLink + forceManyBody + forceCenter, frozen settled nodes, max 300 iterations. |
