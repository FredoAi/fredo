...
| 2026-07-08 | Bug #523 cycle 1 / Spec #275 | Coder must use `useMemo` for derived display state instead of `useEffect` + `setState` when the dependency is a value that changes on every render (array `.length`, new object reference, `Date.now()`). Reviewer checklist now includes "Re-render loops" verification. | Bug #523: `StreamStatus.tsx` had `useEffect(() => setState(...), [events.length])` causing "Maximum update depth exceeded" 11+ times. The ECE compositing changes introduced re-keyed deliveries that amplified the re-render cascade. Spec #275: 3 separate re-render loops from the same `useEffect + array.length` pattern. Re-render loops are a recurring failure class (4 incidents across 2 specs). Coder anti-pattern #8 with Wrong/Right examples added. Reviewer checklist item ensures catch before merge. AGENTS.md documents the fix pattern (epoch counter + useMemo). |
...
| 2026-07-08 | #523 (bug cycle 1) | 1 fix PR merged (PR #532). 2 critical bugs: (1) StreamStatus.tsx infinite re-render loop — `useEffect` on `events.length` triggered "Maximum update depth exceeded" 11+ times; ECE compositing deliveries amplified the cascade. (2) Subagent compositing broken — PostToolUse `task` relationship metadata arrived after child `session.created`, child buffers were re-keyed but no "end" delivery emitted for old child key → frontend sidebar showed child as separate session. Fix: (1) replaced `useEffect+setState` with `useMemo` + monotonic epoch counter, (2) ECE `register_relationship` now emits timedOut "end" delivery before re-keying, frontend `useSessionHistory` excludes composited sessions. All 320 cargo tests pass. |
