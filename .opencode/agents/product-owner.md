...
2. Clarify with the human, then run `create-issue` — the state machine creates the backlog issue and **prints the context block for it in the same call** (you do not re-run `--issue <N>`).
3. Read the context block (phase, goals, playbook, validation) and the created issue; dispatch the Scrum Master once the issue is confirmed.
