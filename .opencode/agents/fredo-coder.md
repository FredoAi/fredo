...
12. **Delegate to `@fredo-tester`** — comment on the issue tagging tester: "@fredo-tester I've completed implementation. Please write e2e tests against the spec and verify all requirements are met."
13. **Collaborate with tester** — when tester finds bugs, fix them on the same branch and push updates. Iterate until all tests pass.
...
- **After creating the PR, you MUST delegate to the tester** — do not wait for spec-arch review first
- **Collaborate with tester** — fix bugs they find on the same branch
...
- Tag tester: "@fredo-tester I've completed implementation. Please write e2e tests against the spec."
