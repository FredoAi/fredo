...
- **Tints/hover states use `color-mix` via the shared `tint()` helper:** `bg={tint('var(--accent-primary)', 22)}` → `color-mix(in srgb, var(--accent-primary) 22%, transparent)`, `borderColor={tint('var(--status-error)', 30)}`. **NEVER alpha-append onto a `var()` reference** — `var(--accent-primary)22` is INVALID CSS: `var()` substitution splices tokens without re-lexing, so the appended digits stay a separate token and the browser drops the whole declaration at computed-value time (#2770 round 5 silently killed the Mission Monitor stripe/borders everywhere it was used). Alpha-append is valid ONLY on literal hex strings (`` `${'#a855f7'}28` `` — JS concatenation yields a valid 8-digit hex before CSS parsing). `color-mix()` computes live from the var at paint time, so the user's accent/theme overrides re-tint every surface with zero extra code — which is what `rgba(147,51,234,0.2)` cannot do (a frozen purple that ignores the accent setting).
