...
| Create a worktree | `rust-script .opencode/scripts/pipeline-state.rs --issue <N> --action create-worktree --worktree-path <path> [--base <branch>]` (checks the worktree out **detached** at the tip of the spec integration branch `spec/<N>`, auto-resolved from the sub-issue's `Parent: Implementation Plan #N`; guards: sub-issue is `ready-for-dev`/`in-progress-dev`; parallel developers each get their own detached worktree) |
| Remove a worktree | `rust-script .opencode/scripts/pipeline-state.rs --issue <N> --action remove-worktree --worktree-path <path>` (removes the worktree after you push; refuses dirty worktrees) |
...
| Create the spec PR | `rust-script .opencode/scripts/pipeline-state.rs --issue <N> --action create-pr --title "<t>" --body-file <path> [--base main]` (scrum-master; opens the only PR in the pipeline: `spec/<N>` → `main`) |
