...
| Transition to next phase | `rust-script .opencode/scripts/pipeline-state.rs --issue <N> --agent <you> --action transition [--to-phase <phase>]` (`--to-phase` optional — inferred when the current phase has exactly one legal exit; required for `testing`/`audit`) |
...
| Create the spec PR | `rust-script .opencode/scripts/pipeline-state.rs --issue <N> --action create-pr [--title "<t>" --body-file <path>]` (scrum-master; `--title`/`--body-file` default from the issue; opens the only PR in the pipeline: `spec/<N>` → `main`) |
| Merge a PR | `rust-script .opencode/scripts/pipeline-state.rs --issue <N> --action merge-pr [--pr <PR#>]` (guards: PR open, CI green; `--pr` defaults to the spec's single open PR; always keeps the branch so evidence URLs survive) |
