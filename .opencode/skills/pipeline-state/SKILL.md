...
| Generate work from the plan | **Removed** — `rust-script .opencode/scripts/pipeline-state.rs --issue <N> --action generate-work` prints a removal note; sub-issues + the tester issue were dropped (all work is tracked on the plan issue + the spec branch; the tester posts `## Tests Runs` / `## Evidence` on the feature issue) |
