...
### #2914 run log — round 1 (`spec/2914 @ a5a882b9`, 2026-09-20/21, live)

**Verdict PASS** — F-128..F-142 all PASS (F-135 `unsupported` leg unit-pinned/named; the acoustic leg is a NAMED BLOCKER). Full report + uploaded captures: the `## Tests Runs (round 1)` comment on #2914.

- **F-128 PASS (live).** Settings → Companion ready branch: `[data-testid^="companion-voice-"]` = exactly `companion-voice-model-audio-row`, `-model-audio-status`, `-device-row`, `-device-select`, `-settings-announcer` (+ the enable switch `aria-label="Enable voice input"`). ALL removed testids null (`-model-row/-status/-download/-recheck/-progress/-location`, `-handling-row/-select/-help`, `-engine-status`, `-autosend`, `-model-audio-use-local`, `companion-step-stt-model`, `companion-setup-optional`). Visible text for `Local transcription`/`sherpa`/`Voice input model`/`Streaming Zipformer` = 0; no Voice nav.
- **F-129 PASS (live).** Not-ready wizard (scratch `models_dir`, no GGUF): `companion-step-llama-server` / `-model-files` / `-server-launch`, summary `1 of 3 prerequisites ready` / `1/3`; no STT step, no Optional group.
- **F-130 PASS (live + static).** Raw `stt:state`+`stt:transcript` subscriptions: `stt:transcript` **0** across every session; only `capturing → processing` (`limitMs 30000`). `download_stt_model`/`stt_check_model`/`stt_warm`/`stt_release` → typed `Command … not found`. Source/lockfile sherpa-free (only the ST-2 cleanup literal).
- **F-131 PASS (live).** Hold with NO sherpa model: `{listening:true, phase:"capturing", readyMs:58}`; dot + `launcher-command-model-listening-chip` "Fredo is listening" + cancel/Stop + `release Space to finish`; release → `{phase:"processing"}`; Escape and × → `{listening:false, phase:null}`, mic released.
- **F-132 PASS (live).** Arming gate `voiceEnabled`-only — the first hold armed with zero sherpa model present; never invoked `stt_check_model`/`stt_warm`; no local phase ever.
- **F-133 PASS (live, L4).** `stt_start` → `{deviceName:"stt-feed", sampleRate:16000}`; clip `{format:"wav", sampleRate:16000, durationMs:1600, truncated:false, base64Len:68328}` == the 1.6 s fixture; console `withAudio:true` + `llmChatWithAudio` + streamed reply; UNSET control = Iriun Webcam/48000 + hold-length clip (non-vacuous).
- **F-134 PASS (live, L4 31 s).** Auto-stop `{listening:false, phase:"processing", limitReached:true, limitMs:30000}`; notice `launcher-command-model-limit-status` = "That's the 30-second limit — Fredo has your recording and is responding."; clip `{durationMs:30000, atLimit:true, truncated:false, base64Len:1280060}` = the entire bounded capture.
- **F-135 PASS (live reactive) / `unsupported` UNVERIFIED (named).** `stop_llama_server` → capability `{state:"serverUnavailable", code:"modelAudioUnavailable"}`; `stt_start` blocked `{started:false, code:"modelAudioUnavailable"}` before capture; `-model-audio-use-local` absent, no "Use local transcription" in production. `unsupported` needs a non-audio model (manifest change forbidden) → unit-pinned residual.
- **F-136 PASS (live).** Chip "Fredo is listening" / "Fredo is listening · Ns left" on one line; cleared on `llm-done` with no next `stt_start`; ZERO transcript.
- **F-137 PASS (static) / live outbound block BLOCKED (named).** Zero network/process symbols in `infrastructure/voice/**`; `127.0.0.1` bind; turn URL `http://127.0.0.1:8080/v1/chat/completions`.
- **F-138 PASS (live, scratch `models_dir`).** Legacy half-downloaded dir removed; sibling dir + files byte-unchanged (`196beab9…`, `74dc343e…`, `3002d703…`); absent case a silent no-op on a second boot.
- **F-139 PASS (live).** `Fredo_companion_voice_handling='local'` persisted across restart → hold armed + ran model-audio end-to-end (reply streamed); no dead/blocked state; key read by no code path.
- **F-140 PASS (CI + local).** `Cargo.toml`/`Cargo.lock` sherpa-free; CI `rust-validate` **pass** (14 m 0 s); `pnpm typecheck` clean; `build` 2573 modules; `test:run` 104 files / 1556 tests / 0 failed.
- **F-141 PASS (static).** Docs describe one model-audio path (SETUP `:86,:88`, SECURITY `:59`, FAQ `:146`, ARCHITECTURE `:210`); `stt-engine-selection.md:5` SUPERSEDED banner; `README.md:26` labels it superseded; invariants re-pointed; `.opencode/tests/**` untouched.
- **F-142 PASS (live).** `readyMs` 4/14/16/52/58 ms; console clean; `telemetry_spans` **19,620 rows** newest `2026-09-21T00:59:35Z`; no new persisted key.
- **Console clean** across every leg (only the pre-existing `motion() is deprecated` WARN).
- **Observation (not a FAIL):** `voiceStartErrorCopy` still carries dead `modelMissing`/`modelCorrupt` copy at `LauncherShell.tsx:309-311` — unreachable (backend no longer emits those codes); cosmetic remnant for a follow-up cleanup.
