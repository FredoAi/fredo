...
### Round 2 (spec/2870 @ 6474a5fe) — results (boot fix verified)

- **S-20 PASS (live).** `#root` populated, no import-analysis overlay. Toggle ON → `.fredo-companion-avatar` in the launcher column's first child, `position: relative`, 80×100, no corner `position:fixed` wrapper; command-bar `y` = 485.77 unchanged vs OFF (485.77). Console clean. *Regression note: the entity sits at the seat slot's left edge (centre ≈840 vs launcher axis 960) — 320 px-wide seat wrapper, see `functional.md` F-62/F-66.*
- **S-21 PASS (live).** Toggle OFF → `.fredo-avatar-idle` at the same seat, entity gone, no placeholder; `Fredo_companion_visible="false"`. Console clean.
- **S-22 PASS (live).** Toggle ON → bubble with exact copy `At your service. How can I help?`, auto-hidden at ≈4.47 s (4000 ±500), anchored `position:absolute` above the slot. Console clean.
- **S-23 FAIL (live).** Ctrl+right-click (recipe a) → the placeholder appears with `data-state="away"`/`role="img"`/`aria-label="Fredo is away"` but its measured `offsetWidth` = **320**, `offsetHeight` = 100 (AC requires 80×100). See `functional.md` F-62. Additionally, the promoted defect F-68 (teleport after an idle auto-return) leaves Fredo rendered nowhere.
