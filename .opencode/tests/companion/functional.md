...
> **F-59..F-66 FAIL / UNVERIFIED (blocking boot defect); F-67 backend half PASS, rendered half FAIL.** The tested tip does not boot: `apps/ui/src/features/home/components/launcher/EmptySeat.tsx:4-5` is the ONLY non-test production file in `apps/ui` importing via `@/`, which the Tauri dev/build Vite config (`apps/tauri/vite.config.ts:18`) resolves to `apps/tauri/src` (that dir holds only `main.tsx` — no `shared/`). Vite aborts: `[plugin:vite:import-analysis] Failed to resolve import "@/shared/components/fredo-avatar" from "../ui/src/features/home/components/launcher/EmptySeat.tsx"`; both `main` and `run-cli-terminal` render an empty `#root` + the red overlay. No seat/entity/placeholder/bubble/command-bar/console-loop is observable. F-67: `fredo emit` chat/tool_use → `{"queued":true}`; `telemetry_spans` = 20207 spans, newest `2026-09-13T18:02:09Z`; `chat_rows`/`tool_use_rows` = 1 each under the injected ids — but NO rendered receipt. **Escape:** `pnpm --filter @fredo/ui build` exit 0 (uses `apps/ui/vite.config.ts` where `@` → `apps/ui/src`); the Tauri dev/build path is not CI-covered. Fix: relative imports in `EmptySeat.tsx` (or extend the Tauri `@` alias), then re-run. Evidence: `.opencode/tmp/2870/tests-runs.md` — `## Tests Runs (round 1)`, Verdict **FAIL**.

### Round 2 (spec/2870 @ 6474a5fe) — results (LIVE, boot fix verified)

> The round-1 boot blocker is FIXED (relative imports in `EmptySeat.tsx`; the webview graph boots, `#root` populated, no import-analysis overlay in either window). The round-2 matrix was executed live. **Overall FAIL: F-62 fails a hard AC assertion (placeholder width), and a new blocking lifecycle defect (F-68) makes Fredo disappear.**

- **F-59 PASS (live).** Toggle ON → `.fredo-companion-avatar` is the SEAT entity: parent `.css-1k18mwh` = the launcher column's first child, entity `position: relative`, 80×100 (offsetWidth/Height 80/100), `data-state="idle"`, no `position:fixed` corner wrapper. Role active in place: single-click → `[companion] askForJoke called` + `llm-token` stream ("Why did the programmer quit using Git? …") → `happy`. **Regression note (see F-66):** the entity renders at the slot's LEFT edge (rect x=800, centre ≈840) while the launcher's vertical axis is x=960 — the seat wrapper is 320 px wide (token bug), so Fredo is visually 120 px left of centre.
- **F-60 PASS (live).** Toggle OFF → `.fredo-avatar-idle` (shared `FredoAvatar` SVG) at the SAME seat (`mb="4"` slot preserved), entity gone; `Fredo_companion_visible="false"`. No bottom-right/corner default exists at settle; `grep` of the changed files finds no `{innerWidth−120, innerHeight−160}` corner default (only a test comment references its removal).
- **F-61 PASS (live).** OFF→ON (real `Switch.onCheckedChange`) → bubble present at +61 ms with EXACT copy `At your service. How can I help?`; auto-hid at +4532 ms (hold ≈4471 ms, within 4000 ±500); computed `position:absolute; bottom:110px; left:160px` (zero layout participation, above the slot). No `speechSynthesis`/audio call exists in the turn-on path (grep + code read). Same-task DOM sampling (G-140) + immediate screenshot.
- **F-62 FAIL (live).** Away-within-main (Ctrl+right-click; `markAway` path) → the placeholder renders with the correct a11y hooks (`data-state="away"`, `role="img"`, `aria-label="Fredo is away"`, `pointerEvents:none`) BUT its measured `offsetWidth` = **320**, `offsetHeight` = 100 — the AC requires **80×100**. Root cause: `EmptySeat.tsx:33-34` passes `width={AVATAR_SM.width}` / `height={AVATAR_SM.height}` (numbers) as Chakra `Box` style props; Chakra v3 resolves `width={80}` to the `sizes.80` token → `var(--chakra-sizes-80)` = **320 px** (`height={100}` has no `sizes.100` token, so it falls back to 100 px). The UI/UX spec explicitly specified `width="80px"`. Same root cause makes the seat wrapper (`LauncherShell.tsx:568`) 320 px wide. Reproduced at default AND 700×900.
- **F-63 PASS (live).** Home + ON ⇒ interactive entity; home + OFF ⇒ decorative mascot (never the placeholder); preference OFF ⇒ mascot NOT placeholder (SI-confirmed). Auto-hidden away ⇒ placeholder still at the seat.
- **F-64 PASS (live).** `LauncherCommandBar` input `getBoundingClientRect().y` = **485.77** in OFF, ON-home, and ON-away, and **unchanged after the idle auto-return settle**; at 700×900 = **446.0** in ON-home and away. |Δ| = 0 px across all states (≤1 px). The seat slot is rendered unconditionally; no new scrollbar/clip (`documentElement.scrollHeight == clientHeight`, column `scrollHeight == clientHeight`).
- **F-65 PASS (live, with the round-2 G-138 recipes).** `open_run_cli` → `run-cli-terminal`; Ctrl+right-click INSIDE the terminal → terminal hosts the single interactive Fredo (fixed @460,348); main seat renders the placeholder and NO second Fredo (global count = 1); a 5 s host idle auto-return re-occupies the main seat; consoles clean in BOTH windows. *Known confound observed once:* the first cross-window attempt did not propagate to main (main kept its seat entity); a subsequent repeat and the MCP-bridge `companion-teleport` emit both propagated correctly — noted as a timing/mount anomaly for investigation (not scored as the AC verdict).
- **F-66 PASS with two named findings (live + static).** EmptySeat is token-native: border `1.5px dashed var(--text-subtle)`, background `tint('var(--accent-primary)', 5)` (computed `color(srgb … / 0.05)`); zero hex/rgba literals and no `var(--x)NN` alpha-append in the changed files; EmptySeat is STATIC (no animation/transition/keyframes) → reduced-motion-safe; no `Maximum update depth exceeded`; no new scrollbar/clip at default or 700×900. **Finding 1 (design regression):** the 320 px seat wrapper (see F-62 root cause) renders Fredo 120 px left of the launcher centre (BEFORE tip: mascot centred at x≈958; AFTER: x≈838) — the UI/UX "delivered at the centre seat" contract is not met. **Finding 2 (named blocker):** the reduced-motion live leg is not drivable on this host (driver cannot flip `prefers-reduced-motion`; #2850 F-19 / #2854 F-38) — static-CSS verification only; `SpeechBubble.tsx` does NOT implement `useReducedMotion()`, so the welcome bubble's spring is not fade-only under reduced motion (static finding, no live PASS claimed).
- **F-67 PASS (live).** `telemetry_spans` = **20774** rows, `max(ingested_at)` = `2026-09-13T18:27:46.933Z`; `fredo emit` chat → `chat_rows` row `e2e-2870-q17b-chat` with `user_message` = the emitted text; tool_use → `tool_use_rows` row `e2e-2870-q17-tool`/`read_file`/`init`; rendered receipts = the round-2 AFTER screenshots + measured DOM geometry.
- **F-68 (NEW — promoted from exploratory) FAIL (live).** **Fredo disappears after a teleport that follows an idle auto-return.** Repro: companion ON at the seat → Ctrl+right-click (teleport away) → wait 5 s (auto-return: seat re-occupied) → Ctrl+right-click again → the seat shows the EMPTY SEAT and **no Fredo renders anywhere** (`.fredo-companion-avatar` count = 0, overlay absent), and it stays that way indefinitely (>3 min observed; the idle auto-return never re-arms). Root cause: `AUTO_RETURN_SETTLED` sets `isAutoHidden=true` (`CompanionContext.tsx:167-169`); a subsequent `TELEPORT` sets `isAway=true` but does NOT clear `isAutoHidden`, and the overlay gate `if (!isAway || !isInThisWindowRef.current || isAutoHidden) return null;` (`FredoCompanion.tsx:185`) then suppresses the overlay, while the idle-timer gate (`CompanionContext.tsx:367`) also requires `!isAutoHidden` → no auto-return. Only a `SET_VISIBLE` (toggle OFF/ON) recovers. Reproduced in the main window (control: a teleport with `isAutoHidden=false` renders the overlay normally) and in the terminal window. Screenshot `after-09-stuck-away-no-fredo.png` + the 20-sample poller log.
