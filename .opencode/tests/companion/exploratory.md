...
### #2853 round 1 findings (spec/2853 @ 1c9696aa, live)

- **E-14 — CONFIRMED (promotes to F-25).** Cleared/`0`/`-5`/`abc` → default 60; `1`→5; `99999`→3600; `30.6`→31. No crash/wedge/console error.
- **E-15 — CONFIRMED (promotes to F-22/R-16).** Terminal-hosted companion: main stayed 0 companion / 0 mascot at t+5.7 s and t+9.9 s; after the terminal's 12 s host timer fired, the `companion-presence` settle mounted the main mascot (920,344) and the terminal companion hid — no ghost across windows.
- **E-16 — CONFIRMED (regression-free).** Toggling OFF mid-countdown then ON showed the companion with a fresh timer; no late auto-return fired against the re-shown companion.
- **E-12 — PARTIAL / UNVERIFIED-with-blocker.** An interaction landing exactly on the deadline was not isolated: when a Ctrl+right-click landed ~0.4 s after the 5 s timer fired, the companion was observed in `teleport-out` (the leave motion) and an interaction cancels an in-flight return per ST-3 — but a clean "interaction at the deadline" probe needs sub-second in-page timing the MCP webview driver cannot provide (long in-page scripts time out). Named blocker: driver latency (~5 s/call).
- **E-13 — UNVERIFIED-with-blocker.** Auto-return with another window covering the launcher was not driven (would require a covering-window fixture); PO accepts the mascot-behind-window nuance so this is not a product risk.
- **E-17 — CONFIRMED (regression-free, partial).** Two OFF→ON toggle cycles each re-armed exactly one timer (return fired once per show, no duplicate); console clean; exactly one Fredo at rest. Full multi-cycle straddling of the deadline not driven (same driver-latency blocker as E-12).
- **NEW FINDING — open TicTacToe does NOT suppress auto-return (FAIL, promotes to F-24).** See `functional.md` F-24 / the round's `## Tests Runs`: the companion returned while the game was open and idle; `CompanionContext.tsx` has no `showTicTacToe`/`isStreaming` gate. This is the round's blocking defect.
