...
- [x] R-16: Window lifecycle (open/close/minimize/restore/focus/maximize), the z-order model (#2825 window frame z=1 > band z 0 covered / 1200 uncovered), and the #2823 Ctrl+Space launcher toggle (opens + focuses searchbox, ESC closes, focus restores) behave EXACTLY as before. The rail stays a PURE READ-ONLY consumer of `useWindows()`/`useWindowActions()` — it never edits `windowStore.ts`/`windowTypes.ts`/the window components; no Rust diff (NFR-1).
  ...
  - **PASS (spec/2841 @ 0852210f, round 1):** Window lifecycle open→minimize→restore/focus→maximize→close all worked through the own kernel (Sessions/Query Viewer/Stepper Probe); z-order preserved (band z 1200 uncovered → 0 covered, window frame z=1). `git diff --name-only main spec/2841` = ONLY 3 source files (AppDock/LauncherChrome/FloatingSettingsButton) + test suites — NO `windowStore.ts`/`windowTypes.ts`/window components edited, NO Rust diff (NFR-1). Rail is a pure consumer of `useWindows()`/`useWindowActions()` (AppDock.tsx:92-93) and dispatches only `focusWindow`/`closeWindow`.

## R-17 — The single top-right LED + advancing clock contract is preserved (no LED regression)

- [x] R-17: Exactly ONE top-right status LED (no bottom LEDs / no second status surface), the clock HH:MM still advances on the 60s interval, and the band still sinks to z0 under a window so the `led-overlay.png` class bug does NOT recur. The ONLY intended change is the cluster's position (centered) — the LED count/size/state-machine and the clock timer must be untouched.
  - **Edge:** exactly one LED trigger in the `<time>` cluster; no `StreamStatus` reintroduced; LED driven by `isOnline` (no pulse).
  - **PASS (spec/2841 @ 0852210f, round 1):** Exactly ONE `status "Online"` LED trigger in the `<time aria-label="HH:MM, online">` cluster (a11y snapshot: `status Online [live=polite]`), ZERO bottom LEDs (`bottomCenterRadiusDots` not present — StreamStatus removed in #2830). Clock HH:MM retained and advancing (11:04→11:15 across the round); band z sinks 1200→0 under a maximized window; `led-overlay.png` class bug did NOT recur (controls hit-test self, F-18).

## R-18 — Changed files token-native; no `var(--x)NN`; Chakra v3 only

- [x] R-18: The changed files (dock/AppDock.tsx, dock/DockEntry.tsx, launcher/LauncherChrome.tsx, settings/FloatingSettingsButton.tsx, Home.tsx) carry ZERO hardcoded hex/`rgba(`/`rgb(`/`hsla(` and NO `var(--x)NN` alpha-append (#2770); colors flow token → CSS var → `tint()`/`color-mix`; Chakra v3 API only (no v2 `isDisabled`/`colorScheme`/`NativeSelect`). The `FloatingSettingsButton.tsx:31` `rgba(0,0,0,0.2)` box-shadow is a KNOWN in-scope literal that MUST be converted (this is the intended fix, not a regression).
  ...
  - **PASS (spec/2841 @ 0852210f, round 1):** Static grep of the 5 changed files → ZERO true color literals (all matches are comment issue-refs `#2838`/`#2825`/`#2819`/`#2830`/`#2821`/`#2770` + the comment word "rgba shadow"); NO `var(--x)NN` alpha-append. The in-scope literal converted: `FloatingSettingsButton.tsx` box-shadow is now `tint('var(--border-color)',30)` (live: `color(srgb .../0.3)`), plus a `tint('var(--accent-primary)',40)` `_focusVisible` ring. Destructive close uses `tint('var(--status-error)',14)` (DockEntry.tsx:199), NOT `variant="outline" colorPalette="red"`. Chakra v3 only. Live computed-style verified in BOTH dark + light (F-20).

## R-19 — The resting-visible rail does not break the covered/peek coexistence

- [x] R-19: When `coveredByWindow === true` (a maximized window covers the desktop), the rail reverts to the exact #2838 peek-only model (off-canvas at rest, slides in on ≤6px left-edge cross) so the window stays full-bleed — honoring the #2825 chrome-vs-window painting rule. When `coveredByWindow === false` (clean desktop), the rail rests visible. The transition between the two must not leave a stale state or a re-render loop.
  ...
  - **PASS (spec/2841 @ 0852210f, round 1):** Covered (maximized Query Viewer): rail `visibility:hidden`, `x:-60`, `matrix(1,0,0,1,-60,-27)` off-canvas — window full-bleed, no persistent rail over it. Clean (minimized/all-closed): rail `visibility:visible`, `x:0`, resting transform. Clean→covered flip (open a window) and covered→clean flip (close) both resolved without a stale state; multiple open/minimize/close cycles produced no `Maximum update depth exceeded` (console clean).
