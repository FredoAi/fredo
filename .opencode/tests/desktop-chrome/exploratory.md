...
- [x] E-19: Change the WebView2/OS zoom (80% / 125% / 150%) with the cluster visible — does the rendered `topMargin`/`rightMargin` scale proportionally and still read as a clear gap from the frame rule, or does it clip/drift? Record the cluster rect + `innerWidth` at each zoom.
  - **FINDING (spec/2872 @ a4074073, round 1) — no defect.** CSS zoom 0.8 (`document.documentElement.style.zoom`, `devicePixelRatio:1`): rendered margins scaled proportionally `top 20→16`, `rightMargin 24→19.19` (×0.8), preserving the asymmetric favour-right (3.19px at 0.8). The cluster stays anchored top-right and fully in-viewport (rightMargin 19.19 > 0) — no clip, no drift. Expected CSS-zoom behaviour; promoted as informational, not an F- case.
- [x] E-20: Rapidly switch theme presets with a maximized window under the cluster (band sunk to z=0) — does the cluster ever paint above the titlebar min/max/close controls during the transition, or flicker at the new inset? Any console error is a finding.
  - **FINDING (spec/2872 @ a4074073, round 1) — no defect.** 6 rapid preset switches (`light-default` → `dark` → `tokyo-night` → `solarized` → `cyberpunk` → `arctic`) with a maximized Query Viewer (header full-bleed w=1918): rendered `topMargin:20 / rightMargin:24` IDENTICAL in every preset (no re-flow/flicker at the new inset); band z stayed **0** throughout (never rose above the z=1 window stack); no console error. Promoted as informational, not an F- case.
