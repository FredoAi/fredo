...
- [x] R-22: The rework touches only `PixelButler.tsx` + any new avatar helper (git diff scope). The avatar container (`Box mb="4"`, LauncherShell.tsx:501), the command bar, the app grid, the keyboard hints, the clock/LED chrome, and the open/close lifecycle are UNCHANGED — no layout shift of the avatar box or the surrounding chrome at the declared display size or at narrow widths. Reference #2808 F-4 + R-15.
  - **PASS (live + git, spec/2837 @ 93c8379c).** `git diff main spec/2837 --stat` touches exactly 3 files: `PixelButler.tsx` (rewritten), `fredoAvatarGeometry.ts` (new), `__tests__/fredoAvatarGeometry.test.ts` (new) — `LauncherShell.tsx` (wrapper `Box mb="4"` :501) is UNTOUCHED. Live: avatar renders centered above the command bar at 132×165 (flex-column centering held, delta 0 px); command bar unchanged (`input[role="searchbox"]` at same geometry); grid/hints/clock chrome unchanged in the DOM. Narrow 700×900: no layout shift, avatar re-centers, chrome intact.

## R-23 — Token-native color baseline retained

- [x] R-23: Changed launcher files carry ZERO hardcoded `#[0-9a-fA-F]{3,8}` / `rgba(` / `rgb(` and NO `var(--x)NN` alpha-append; the figure is a SINGLE accent fill driven by `var(--accent-primary)` (hue follows the LIVE accent token — cyan under `light-default`/`dark`, purple under the `classic` base; do NOT fail on hue when a non-cyan accent is active) and any added glow is token-based (`tint()`/`color-mix`/var filter); no cross-feature import (imports only React); no re-render loop (console clean of `Maximum update depth exceeded`). Reference F-6/R-5/R-9/R-12/R-14/R-16 + F-24 + R-19.
  - **PASS (static + live, spec/2837 @ 93c8379c).** Grep of `launcher/**` for `#[0-9a-fA-F]{3,8}`/`rgba(`/`rgb(` → ZERO true literals (all matches are comment issue-refs); `var(--x)NN` alpha-append → ZERO. `fredoAvatarGeometry.ts` imports nothing; `PixelButler.tsx` imports only React + the geometry module — no cross-feature import. Live re-theme (shipped `ThemePresetSelector` in Appearance): avatar = `rgb(0,209,209)` cyan under `light-default` + `dark`, `rgb(147,51,234)` purple under `classic` base (Default/None), `rgb(0,255,65)` under Matrix — hue follows the LIVE accent token in every preset (token-derived, zero hardcoded color). Console clean of `Maximum update depth exceeded` across the full run (theme×5, resize, minimize/reopen, settings cycles).

## R-24 — Avatar still renders at the Architect-bound display size in the launcher

- [x] R-24: The avatar renders at the Architect-bound display size (`DISPLAY_WIDTH` ∈ [112, 168] px, recommended 132×165, aspect 1014:1264), undistorted, centered above the command bar, fully visible (no clipping/overflow) in BOTH a light preset and the dark base via the shipped `ThemePresetSelector`. Reference #2827 F-30/E-20 + #2824 F-23 + #2837 F-39b.
  - **PASS (live, spec/2837 @ 93c8379c).** Live `getBoundingClientRect` = 132×165 CSS px (DISPLAY_WIDTH 132 ∈ [112,168]; DISPLAY_HEIGHT round(132·1264/1014)=165 ∈ [128,210]), aspect 0.8 (1014:1264 viewBox `0 0 1014 1264` — undistorted), centered (centerX 960 == viewport 960, delta 0), `fullyVisible: true`, command bar below at y 550.77 (no overlap). Verified in `light-default` (white surface) AND `dark` base + `classic` base — same box in every preset. Screenshots `ac5-light-default.png`/`ac5-dark.png`/`ac5-classic-purple.png`.

## R-25 — Launcher interaction surface unchanged

- [x] R-25: The #2823 Ctrl+Space toggle, the #2824 ESC keycap/hints, and the #2830 single-LED cluster are unchanged by the avatar rework (reference launcher R-10..R-21 + desktop-chrome R-7..R-14). The avatar remains decorative (`aria-hidden`).
  - **PASS (live + git, spec/2837 @ 93c8379c).** Git diff scope = only `PixelButler.tsx` + geometry module + test — the Ctrl+Space handler, ESC keycap/hints, and LED cluster files are untouched. Live: `svg aria-hidden="true"` retained (decorative — no role/focus/pointer on the avatar); the single top-right LED (`data-testid="desktop-status-led"`) is present and unchanged; the resting launcher surface (notch + command bar + grid) renders normally after minimize/reopen cycles; console clean.
