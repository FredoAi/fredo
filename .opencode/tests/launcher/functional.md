...
- [ ] F-21: Read `C:\Code\fredo\.opencode\wireframes\bugs\esc-close.png` (explicit path), render the launcher ENGAGED (focus the command bar OR enter a query so the bottom `ESC CLOSE` hint is visible) on the `spec/2824` build at 100% OS scale, capture a screenshot, and compare side-by-side. **Expected:** the ESC keycap is a small, padded, rounded badge — `ESC` text inside a rounded-rect with a visible text→border padding gap, NOT touching/overlapping the launcher frame or any border. Any deviation (border-touching glyph, missing padding, missing rounded corners, wrong size) ⇒ FAIL. A `main`/old-build capture is rejected (`live`).
  - **FAIL (round 1 — `spec/2824` live).** The border-touching defect IS fixed (padded + rounded + no frame/touch contact), BUT the badge is **oversized**: it renders **75×35 px** (measured live via `getBoundingClientRect`), the plan's UI/UX target geometry was **~34×17 px** and the reference-accurate size at live scale is **~38×19 px** — a **~2× deviation**. Root cause: `px={7}`/`py={3}` in `LauncherChrome.tsx:107-108` resolve to Chakra **space tokens** (`space.7` = 28px, `space.3` = 12px), NOT the intended raw `7px`/`3px`. Scale-independent geometry (anchored to the 11px CLOSE label): badgeH/closeH = **3.18** (reference = **1.75**), badgeW/closeW = **2.14** (reference = **0.875**); ESC inner text fills only **~26%** of badge height (reference = **79%** — a tight keycap). Live dark-leg measurement: `{ badge: 75×35, borderRadius:5px, border:1px solid var(--text-primary)=rgb(229,231,235), bg:var(--card-bg)=rgb(21,26,33), gap-to-CLOSE:8px, frame-gaps: 8px bottom / 55px right }`. Reference PNG pixel analysis (read from `.opencode/wireframes/bugs/esc-close.png`, 127×74): badge region 28×14, ESC inner text 22×11 (fills 79% of badge height), gap-to-CLOSE 8px. Screenshot comparison: `ac1-engaged-dark.png`, `ac1-side-by-side.png`, `ac1-zoom-probe.png`. **AC1 = FAIL (oversized keycap).**
...
- [ ] F-25: With the launcher open showing the close hint, confirm the keycap still reads as a clean keycap affordance — the reference-image check reconfirmed on the fix build. **Expected:** the keycap is a clean, padded, rounded badge (no regression to the old border-touching/cramped look); `↑↓ NAVIGATE` / `←→ SELECT` and ESC functional behavior (out of scope) unchanged.
  - **PASS (round 1 — live, spec/2824).** The keycap renders as a clean, padded, rounded badge — the old border-touching/cramped SVG look is gone (ESC text has a clear 7px/3px-ish inset from its outline; borderRadius 5px). The `↑↓ NAVIGATE` / `←→ SELECT` hints and their arrow glyphs (still `currentColor` SVGs) are unchanged; the frame geometry (12px inset, 12px radius) and the hint-row layout (space-between, pin-right CLOSE) are unchanged. Both edges verified: (1) **fresh launcher reopen** — ESC → idle (grid collapses, only the searchbox remains) → re-focus/query re-engages → badge re-measured **75×35**, still padded/rounded, border crisp (arrow SVGs present); (2) **theme switch** — light↔dark while engaged re-tints the badge with no re-regression. **Note:** F-25's "clean, padded, rounded" criterion PASSES, but it shares the same keycap with F-21, whose **oversized** discrepancy (75×35 vs reference-accurate ~38×19) is the AC1 FAIL — F-25 measures affordance health (clean/padded/rounded/no-touch), which holds; only the size deviates, and that is AC1's gate. **AC5 = PASS.**
