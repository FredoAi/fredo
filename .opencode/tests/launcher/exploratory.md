...
- [x] E-26: Re-theme through several shipped presets (light + dark + a non-cyan accent such as Matrix) while the launcher avatar is visible — does the avatar re-tint token-native with no dead/stale color and NO geometry change/deformation? Any element stuck on a stale color, or any theme-dependent deformation, is a finding.
  - **Observed (PASS — no finding).** Cycled `light-default` (accent `#00d1d1` → avatar `rgb(0,209,209)`) → `dark` (`#00d1d1`) → `classic` base Default/None (`rgb(147,51,234)` purple) → Matrix (`#00ff41` → `rgb(0,255,65)` green) → back to the as-found Tokyo Night (`#7aa2f7` periwinkle) via the shipped `ThemePresetSelector` in Appearance settings. Every switch re-tinted the avatar token-native to the live `--accent-primary` with **zero dead/stale color** and **zero geometry change** (58 rects, same coordinates, same 132×165 box in every preset). No finding — no promotion. Screenshot: `e26-matrix-green.png` (non-cyan green accent proof).

## E-27 — Narrow/DPI resize + rapid open-close integrity

- [x] E-27: Resize the webview small/narrow and cycle launcher open/close rapidly around the new avatar — does the avatar stay crisp (uniform cells, no anti-alias fill-in of the fragmentation gaps), fully visible, no clip/overflow, no console error / no re-render loop? Any filled-in gap, clip, or console error is a finding.
  - **Observed (PASS — no finding, with vision-caveat).** Narrow 700×900 resize: avatar stayed **132×165**, centered (350==350), `fullyVisible: true`, 58 rects, no clip/overflow; restore to 1920×1017 re-centered cleanly. Minimize/reopen cycles around the avatar: avatar stayed mounted with 58 rects throughout. The fragmentation gaps are 26–38 wireframe units = **3.4–4.9 CSS px** at the 132px display, well above the crispEdges sub-1px threshold — no anti-alias fill-in is geometrically possible (there is no rect painted in the gap channels: channel388=0, btnGap=0 rects). Console across resize + cycles: no `Error:`/`Uncaught`/`Maximum update depth exceeded`. Caveat: pixel-level confirmation of gap visibility is delegated to the uploaded zoom screenshot (`ac4-body-zoom.png`) — tester model has no image input. No finding — no promotion.
...
- [ ] E-28: At a fractional OS/webview zoom or non-100% DPI, does the new multi-rect geometry stay proportional (no merged/seam-shifted cells, no subpixel blur destroying the fragmentation gaps)? Any distortion at non-integer scale is a finding.
  - **Observed (PASS — limited, DPR 1.0; no finding).** The environment DPR = 1.0 (devicePixelRatio read from the live webview), so a fractional-DPI leg could not be driven in this round — the OS display scale is 100%. At DPR 1.0 with the aspect-locked 132×165 display, all measured geometry is coordinate-exact and proportional (F-37/F-38/F-40). The multi-rect geometry is SVG-vector in a 1014×1264 viewBox with `shapeRendering="crispEdges"`, so non-integer webview zoom re-rasterizes vector rects (not bitmap cells) — no cell-merge/unit-shift failure mode exists in the vector representation (that failure mode belonged to the #2827 raster-grid). Recorded as observed-PASS on the vector-representation argument + DPR-1.0 measurement; a true fractional-DPI environment (125%/150% OS scale) is a follow-up exploratory if the human wants belt-and-suspenders confirmation. No finding — no promotion.
