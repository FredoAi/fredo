...
- [x] E-18 (OBSERVED 2026-09-07 #2835): Sustained unbounded-growth probe — heap sampled every ~15-22 s through a live opencode stream + replay. On the AFTER leg heap plateaued in-window (1,380→1,343 MB, GC dips) but at ~1.3-1.4 GB absolute after the full replay; no monotonic per-window growth observed, but the per-open full replay re-inflates to ~0.9-1.4 GB. Before leg: heap climbed past 1.4 GB and stayed high.
- [x] E-19 (OBSERVED 2026-09-07 #2835): Burst/large-replay — first-open measured on the real corpus (≈14.1k chat / ≈17.5k tool rows): BEFORE ≈45.5k envelopes / 94-102 batches; AFTER ≈31.5k envelopes / 64-69 batches. AFTER still exceeds the ≤40-batch budget and the first row takes 10-18 s (both legs) because the full real table re-drains on every open.
- [x] E-20 (OBSERVED 2026-09-07 #2835): Rapid-switch — toggled selections during live streaming on the AFTER leg: canvas followed the selected session (no stale/wrong-session graph), no freeze. On the BEFORE leg the main thread was periodically blocked (execute_js probes timed out) during replay.
- [x] E-21 (OBSERVED 2026-09-07 #2835): Rapid feature open/close — a warm reopen on the AFTER leg re-ran the FULL replay (~31.5k envelopes again; heap back to 1,328 MB). Every mount (cold or warm) pays the full-table replay → per-mount cost confirmed.
- [x] E-22 (OBSERVED 2026-09-07 #2835): High session-count + streaming — with ~4 listed sessions and live streaming, AFTER interactions stayed responsive; the enormous per-open cost is the full replay + per-flush-batch derive, not per-session O(n²) list work.
- [x] E-23 (n/e 2026-09-07 #2835): delete-while-stream not exercised this round (out of the perf scope; covered by prior ghost/deletion legs).
- [x] E-24 (PASS 2026-09-07 #2835): Console check on every probe — clean (no Error/Uncaught/Maximum update depth) on both legs; only pre-existing WARNs (motion deprecation, cache-reconciliation fallback for one corpus session on the pre-fix leg).
