...
- [ ] F-1 (REQ-1): Drive a normal opencode session via Run CLI (free model, minimal tree, marker in first prompt). Wait for the telemetry store to confirm its rows landed, then select it in Mission Monitor's session list and snapshot the canvas (DOM + screenshot).
...
- [ ] F-2 (REQ-1a): Select a listed session that lands zero visible graph nodes (ghost / transitional / subagent-only).
...
- [ ] F-3 (REQ-2): Inspect the session list for a session with landed telemetry rows + zero rendered graph nodes.
...
- [ ] F-4 (REQ-3): Select a session immediately after launch before its first spans land (zero nodes, zero landed spans).
...
- [ ] F-5 (REQ-3a): For a landing-in-progress session, observe zero spans → landed rows → content-or-explanation.
...
- [ ] F-6 (REQ-3b): Drive two sequential fixtures; confirm each resolves consistently.
...
- [ ] N-1 (NFR-1): With several listed sessions, verify the session-list derivation is a single map pass (no per-session rescan) and the list does not lag/block.
...
- [ ] N-2 (NFR-2): After each interaction, read the webview JS console (`tauri_read_logs source="console"`).
...
- [ ] N-3 (NFR-3): Verify no graph-node layout/color/edge change, no ingestion/storage/classification change, no v1 hydration/fallback reintroduced.
