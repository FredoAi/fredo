...
- [ ] R-18: **Compact single-line node bars do not change underlying per-message values (Spec #2711 invariant on the #2723 surface).** The compact single-line format (AC2) must NOT re-base or re-derive node values: every node's figures still equal its own per-message span usage (node-vs-span authoritative, zero tolerance; Style B MSG2 = real value, never a delta artifact). The top session bar (AC1) must not change the per-node values either — the value pipeline is untouched, only the surface. Evidence: node readouts vs per-message span query, both provider styles where available. Expected: identical values as the pre-#2723 surface for the same session.
