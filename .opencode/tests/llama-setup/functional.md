...
## #2871 extension — command-bar chat reuses the managed llama-server path

> Issue #2871's reply is generated by the SAME managed `llama-server` (`llm_chat` → streamed
> `llm-token`s → `llm-done` → additive `llm-error`) — no new model management, install, or
> window. These rows extend the #2857 launch/round-trip surface. **G-136:** F-38/F-39 (the
> streamed-class reference) remain in force; this is additive. Live policy — REAL server only.

## F-64 (REQ-3/REQ-4) — Bar chat uses the existing managed transport (no new path)

- [ ] F-64: On a ready host (`/health` 200 on 127.0.0.1:8080), companion ON; send a long prompt
      from the launcher command bar; sample the reply DOM across the window.
  **Expected:** the prompt reaches the SAME managed server and the reply streams back via the
      existing `llm_chat`/`llm-token`/`llm-done` transport — ≥2 distinct partial contents over
      time then the final text (G-130: reproduce the streamed class); no new command name, no new
      model install/management, no new window/surface.
  - **Edge:** server `starting` (not healthy) → the readable not-ready path, no hang; server
    unreachable → one readable error line; multi-second TTFT; abort mid-stream.

## F-65 (REQ-LIVE) — Live telemetry + managed-server receipt for the bar-chat run

- [ ] F-65: Same run as F-64: `fredo emit --event-type chat --session-id e2e-2871-chat` +
      `--event-type tool_use --session-id e2e-2871-tool`; query `telemetry_spans` + row tables;
      keep the screenshot/DOM receipts.
  **Expected:** `telemetry_spans` NON-ZERO with a recent `max(ingested_at)`; the injected events
