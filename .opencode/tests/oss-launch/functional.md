...
- [ ] F-8 (AC4b): audit.yml triggers — weekly `schedule:` cron present AND a path filter covering `**/Cargo.lock`; rest of file unchanged vs `git show <fork-sha>:.github/workflows/audit.yml`. **Edge:** cron must be genuinely weekly; path pattern must match `apps/tauri/src-tauri/Cargo.lock`.
