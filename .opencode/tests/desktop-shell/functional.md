...
- [x] F-8: Focusing the search-or-command bar (click or Tab) reveals the feature grid BELOW the bar (`#fredo-launcher-grid` with the `SHOWABLE_FEATURES` tile set = Mission Monitor / Query Viewer / Run CLI / Stepper Probe) plus the keyboard-hints row (↑↓ NAVIGATE · ←→ SELECT · ESC CLOSE).
  - **QUERY-reveal leg PASS (#2819 round 1):** typing a query reveals `#fredo-launcher-grid` (aria "Apps") with all 4 SHOWABLE_FEATURES + hint row. **FOCUS-reveal leg UNVERIFIED (G-053 driver gap):** the WebView2/MCP driver cannot emit a trusted DOM `focus` event — programmatic `.focus()` sets `document.activeElement` but dispatches no `focus`/`focusin` (verified on the searchbox AND the notch; `nativeFocusFired:false`, `aria-expanded` stays `"false"`), and `interact` click/`focus` + keyboard `Tab` don't advance focus because the OS window isn't foreground (MCP plugin <0.13 → `tauri_manage_window focus` refused). `handleBarFocus → setEngaged(true)` present in `LauncherShell.tsx:139-145` but only source-inspectable here. Not claimed a product defect.
...
- [x] F-12: The tester performs a SIDE-BY-SIDE screenshot comparison against `desktop-light.png`, attaches the comparison image to the evidence, and calls out EVERY deviation (element present/absent, position, size, color, accent, font, spacing).
  - **FAIL (#2819 round 1).** Side-by-side vs `desktop-light.png` (1536×1024). **Unmapped IDLE-LIGHT deviations:** (1) **vertical placement** — the live avatar+command bar are clustered near the TOP (avatar y64 ~7.9% H, bar y120 ~11.8% H) whereas the wireframe places them upper-middle/center (wireframe teal avatar rows 315–434 ~36.6% H, bar text rows 508–520 ~50% H); (2) **avatar size** — live 32×32 vs wireframe ~119px drawn / UI-UX spec target ~48px; (3) **avatar→bar gap** — 24px live vs ~40px wireframe. Matched: frame (inset12/radius24), notch (top-center poking frame), clock (top-right ONLINE), side-ticks (x≈0.058W), dot-grid+`+` (x≈0.928W), accent token `#00d1d1`. Engaged leg composition matches (grid below bar, hints `↑↓ NAVIGATE`/`←→ SELECT`/`ESC CLOSE` bottom, teal scroll-thumb) with the authorized Fredo `SHOWABLE_FEATURES` tile set; glyph checkpoint PASSES (LeftRight arrows, not Enter/↵). Screenshots: `.opencode/evidence/2819/f12-idle-light-compare.png`, `f12-engaged-dark.png`. **F-12 = FAIL** per the hard gate.
