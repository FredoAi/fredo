...
- [ ] S-7: **ghostty surface present (quick path).** DOM snapshot of the terminal window shows the ghostty terminal surface and NO `.xterm` classes. Quick smoke — full assertions live in F-3. **FAIL** — Ghostty canvas present but loading overlay persists (event routing bug). Evidence: Spec #2728 round 1 `## Tests Runs`.

- [ ] S-8: **Terminal window auto-closes on exit (quick path).** Type `exit` in the terminal; `tauri_manage_window(action="list")` shows zero terminal windows shortly after. Quick smoke — full assertions live in F-7. **PARTIAL** — Manual Stop closes window. Auto-close unverifiable. Evidence: Spec #2728 round 1 `## Tests Runs`.

- [ ] S-9: **Working directory honored (quick path).** Run `pwd` in the terminal; output matches the configured `run_cli_work_dir` (or the home fallback when unset). Quick smoke — full assertions live in F-6/R-7. **PARTIAL** — Toolbar shows correct dir. Cannot verify `pwd` output. Evidence: Spec #2728 round 1 `## Tests Runs`.
