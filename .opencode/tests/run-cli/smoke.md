...
- [ ] S-3: **Run CLI surface reachable** — the "Run CLI" entry (Launcher/Menu) renders and is clickable.
...
- [ ] S-6: **Run CLI opens exactly ONE window (quick path).** Click "Run CLI"; `tauri_manage_window(action="list")` immediately shows main + 1 terminal window and stays at main + 1 through launch. Quick smoke — full assertions live in F-1/F-2.

- [ ] S-7: **ghostty surface present (quick path).** DOM snapshot of the terminal window shows the ghostty terminal surface and NO `.xterm` classes. Quick smoke — full assertions live in F-3.

- [ ] S-8: **Terminal window auto-closes on exit (quick path).** Type `exit` in the terminal; `tauri_manage_window(action="list")` shows zero terminal windows shortly after. Quick smoke — full assertions live in F-7.

- [ ] S-9: **Working directory honored (quick path).** Run `pwd` in the terminal; output matches the configured `run_cli_work_dir` (or the home fallback when unset). Quick smoke — full assertions live in F-6/R-7.
