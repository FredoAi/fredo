...
> Full evidence: the #2896 `## Tests Runs` (round 3, Verdict PASS). Both round-2 FAILs (RC-1 backfill never completed; RC-2 phantom no-op write) are fixed and verified live; the S6/S4 blockers are unchanged named blocks.

- [x] F-1/F-3 (REQ-1, **PASS 2026-09-19 #2896 round 3**): the declared store now holds the whole qualifying corpus (31 rows) and survives a cold restart intact (`r3-change-A` customName persisted; `last_version=181`, `backfill_done=1`) with **no** backfill re-drain.
- [x] F-6 (REQ-2 sibling-field isolation, **PASS 2026-09-19 #2896 round 3**): re-driven on the SI-B disposable DB — `feature_data_write {customName:'gran-a'}` fired the table watch + record watch + `fields:['customName']` watch; the sibling `fields:['latestAt']` watch received **ZERO**. A subsequent canonical chat response that made the group cease qualifying emitted `kind:"remove"` (`values:null`, `changedFields` = the known fields) to the table/record watches and `changedFields:['latestAt']` / `['customName']` to the respective field watches — removal is delivered to every watch on the record per contract (e).
- [x] F-10 (REQ-3 no-op edge, **PASS 2026-09-19 #2896 round 3** — RC-2 fixed): identical-value `feature_data_write {customName:'noop-test'}` → `{"updated":0}`, **zero** new `featureBatch` notifications, `last_version` unchanged (180); writing the same current value again after a real change → `{"updated":0}`, zero notifications, `last_version` still 181. A real change (`noop-test`→`r3-change-A`) → `{"updated":1}`, one delivery per open table watch with `changedFields:["customName"]`, `values.customName='r3-change-A'`, `version:181`; row `_row_version` 1→2 and `_updated_at` advanced. A real value diff is required before a notification.
- [x] F-14/F-18 (REQ-4 Scenario B + first meaningful paint, **PASS 2026-09-19 #2896 round 3**): A-14 Δ pair measured live — large corpus (31 sessions / 30,320 chat rows) Δ = 39.8 ms and 92.7 ms; small corpus (3 sessions / 3 rows) Δ = 62.4 ms; bound satisfied (see mission-monitor N-16).
- [x] F-15 (REQ-4 isolation, **PASS 2026-09-19 #2896 round 3 — via a FRESH `qa2896r3probe` scope on the SI-B disposable DB, never the real corpus**): declare → `feature_qa2896r3probe_probes` created; cross-namespace reads refused both ways with hard named errors (`feature 'qa2896r3probe' has not declared table 'sessions'`; `feature 'mission-monitor' has not declared table 'probes'`); the probe's own read returns its own (empty) rows only. The real corpus after restore carries only `mission-monitor.sessions` (the probe scope was discarded with the disposable DB).
- [x] N-4 (NFR-4, **PASS 2026-09-19 #2896 round 3**): isolation + idempotent create across restart confirmed on the restored real corpus (single persisted declaration, `backfill_done=1`, 31 rows).
- [x] N-5 (NFR-5 console, **PASS 2026-09-19 #2896 round 3**): console clean throughout (only the pre-existing `motion()` WARN + MM auto-fit DEBUG).
- [x] N-6 (NFR-6 theme, **PASS 2026-09-19 #2896 round 3**): no hardcoded colors introduced; MM surfaces render from `var(--card-bg)` / `var(--border-color)` / `var(--text-secondary)` / `var(--status-error)`.
- [ ] F-17 (REQ-3/S6, **UNVERIFIED 2026-09-19 #2896 round 3** — named blocker, unchanged): the failure/disconnect surface cannot be driven live — the disposable-DB DDL lever is sandbox-denied (`sqlite3 … "DROP TABLE …"` → permission denied; only `sqlite3 -readonly` allowlisted) and there is no stream-health signal. Hook-level named rejection remains live-verified.
- [x] F-19 (round-1 promoted finding, **FIXED and verified 2026-09-19 #2896 rounds 2–3**): the legacy `feature_mission_monitor_sessions` is quarantined as `__legacy_20260919071609` (never dropped), the declared schema is real, and the one-time backfill now completes (31 rows, `backfill_done=1`).
