...
- [ ] S-4: Adapter/ECE unit suite compiles and passes - `cargo test` in `apps/tauri/src-tauri` (adapter modules `opencode.rs` + `otlp.rs` + `parent_prompt_cache.rs` + `contract/` + `telemetry/`) exits 0
