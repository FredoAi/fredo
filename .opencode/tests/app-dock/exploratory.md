...
## E-9 — Active-indicator axis per orientation (PROMOTED to functional F-13, round 2)

- [x] E-9: With a window focused+non-minimized, does the ACTIVE dock entry's well render the correct active-indicator AXIS per orientation — Sidebar left-edge bar `inset 3px 0 0 0 var(--accent-primary)` vs Bottom bar bottom-edge underline `inset 0 -3px 0 0 var(--accent-primary)` (wireframe)? **PASS (round 2, promoted to F-13)** — live computed-style verified both axes (see functional.md F-13). Round-1 FAIL (Bottom used the left-edge bar) resolved by the FD-2 orientation-branch fix.

## E-10 — Bottom bar tooltip placement (PROMOTED to functional F-12, round 2)

- [x] E-10: Does the Bottom bar tooltip open ABOVE the well (`data-placement="top"`), never `right`/clipped at the viewport bottom? **PASS (round 2, promoted to F-12)** — popper resolves `data-placement="top"`, rect above the well, not clipped (see functional.md F-12). Round-1 FAIL resolved by the FD-3 `strategy:'fixed'`+`flip:false` fix.
