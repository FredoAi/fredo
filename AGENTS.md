...
- **Tint/hover variants append alpha to the var** — `var(--accent-primary)22`, `var(--status-error)15` — NOT `rgba(147,51,234,0.2)` (a hardcoded alpha purple that ignores the user's accent choice).
...
- **Never Grep the installed plugin path (`~\.config\opencode\plugins\fredo.js`)** — the Grep tool stalls on the `~` home-dir path on Windows (observed #2770 rounds 3-4; two tester rounds lost). Verify plugin currency with `Get-FileHash` vs `apps/opencode-plugin/dist/index.js` + `Select-String -LiteralPath` with **bundle-form** anchors — see G-048 in `.opencode/skills/dev-environment/SKILL.md`.
