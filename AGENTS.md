...
- **Session splitting (openocode subagents):** OpenCode creates SEPARATE session IDs for subagents. When viewing the parent agent's session, subagent events are invisible because they carry the subagent's `sessionId`, not the parent's. The adapter extracts the child-to-parent mapping from `tool_response.metadata.parentSessionId` and `tool_response.metadata.sessionId` in PostToolUse `task` events. To make subagent nodes visible in the parent session's graph, use a module-scoped `Map<string, string>` (`childToParentSession`) that maps child session IDs to their parent session ID. Expand `sessionDeliveries` filtering to include deliveries whose sessionId maps to the selected parent session. Suppress child sessions from the sidebar session list by checking the map. NEVER use `useRef` for this map — it must survive React mount/unmount cycles.
- **Subagent output filtering (`filterSubagentOutput` in `contract.ts`):** Real opencode subagent output is a concatenation of system prompt + assistant reasoning + actual response, often WITHOUT newline separators. The function `filterSubagentOutput()` in `apps/ui/src/features/mission-monitor/lib/contract.ts` strips the system prompt and reasoning to leave only the response. It uses a **"last matching sentence"** strategy: split text by sentence boundaries, scan ALL sentences, find the LAST sentence matching instruction verb patterns (`tell`, `return`, `write`, etc.) or reasoning prefix patterns (`the user wants`, `let me`, `i'll just`, etc.), and return everything AFTER that sentence as the response. This strategy works because system prompt + reasoning sentences form a PREFIX — the response starts after the last instruction/reasoning sentence. **Never use a "first non-match = response" heuristic** — system prompts contain non-matching sentences (parent joke context, e2e markers) that trip this gate prematurely. The filter runs progressively on every ECE update delivery (not just at end-lifecycle) because OpenCode does NOT emit `session.idle` events for subagent sessions, so subagents never reach the End lifecycle. Bug #478 fix (PR #494): 14 unit tests in `contract.test.ts` cover all known output formats. When modifying this function, ALWAYS add test cases for real opencode subagent output captured from e2e runs.
