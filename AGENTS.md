...
- **Subagent session merge — adapter-level sessionId rewrite (Spec #509):** OpenCode creates SEPARATE session IDs for subagents. The OpenCodeAdapter rewrites subagent `sessionId` to the parent's `sessionId` at the adapter level via `session.updated` events carrying `properties.info.parentID`. The adapter maintains an internal `Arc<Mutex<HashMap<String, String>>>` child→parent map (capped at 10,000 entries, oldest-first eviction), rewrites `sessionId` on ALL subagent events, and derives `correlationId` from the ORIGINAL child session ID (not the rewritten parent). This means the ECE, ContractEngine, and frontend never see a separate subagent session — all deliveries belong to the parent. The frontend detects subagent deliveries via `deliveryCorrelationId(d) !== deliverySessionId(d)` (child_sid ≠ parent_sid after rewrite) — no frontend mapping, sidebar suppression, or `childToParentSession` Map required. **Always solve data transformations at the source (adapter), not at the consumer (frontend).** This supersedes Bug #478's frontend mapping approach which suffered from chicken-and-egg deadlocks between `sessionDeliveries` filtering and map population.
- **Subagent agent-name filter (Bug #509 cycle 2):** When the OpenCodeAdapter detects a subagent session via `properties.info.parentID` on `session.updated` events, it MUST also check `properties.info.agent` to exclude internal OpenCode tool-execution agent sessions (`build`, `plan`). These sessions are spawned internally by OpenCode to execute tool calls (bash, read, todowrite) in sub-sessions and are NOT user-requested @-subagent dispatches. Without this filter, the adapter rewrites their sessionId, and the frontend creates spurious SubagentNodes for every tool execution (11+ nodes instead of 1). The fix is two-layer: (1) adapter-level check in `opencode.rs:session.updated` handler skips `child_to_parent` population + sessionId rewrite for `build`/`plan` agents, (2) frontend belt-and-suspenders check in `useMissionMonitor.ts:chat-node` init handler skips SubagentNode creation for internal tool agents. **Always verify agent identity when filtering by `parentID` — not all child sessions are user-requested subagents.**
- **Subagent session merge (Spec #509):** OpenCode creates SEPARATE session IDs for subagents. The OpenCodeAdapter rewrites subagent `sessionId` to the parent's `sessionId` at the adapter level via `session.updated` events carrying `properties.info.parentID`. The adapter maintains an internal `Arc<Mutex<HashMap<String, String>>>` child→parent map (capped at 10,000 entries, oldest-first eviction), rewrites `sessionId` on ALL subagent events, and derives `correlationId` from the ORIGINAL child session ID (not the rewritten parent). This means the ECE, ContractEngine, and frontend never see a separate subagent session — all deliveries belong to the parent. The frontend detects subagent deliveries via `deliveryCorrelationId(d) !== deliverySessionId(d)` (child_sid ≠ parent_sid after rewrite) — no frontend mapping, sidebar suppression, or `childToParentSession` Map required. **Always solve data transformations at the source (adapter), not at the consumer (frontend).** This supersedes Bug #478's frontend mapping approach which suffered from chicken-and-egg deadlocks between `sessionDeliveries` filtering and map population.
