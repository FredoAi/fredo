...
- **Session splitting (openocode subagents):** OpenCode creates SEPARATE session IDs for subagents. When viewing the parent agent's session, subagent events are invisible because they carry the subagent's `sessionId`, not the parent's. Populate the `childToParentSession` map from TWO sources: (a) `tool_response.metadata.parentSessionId` and `tool_response.metadata.sessionId` in PostToolUse `task` events, AND (b) `properties.info.parentID` in `session.created` events (spec #461 verified this path EXISTS in real opencode). The `session.created` path fires first, capturing the child-parent relationship before the child is processed as a separate AgentNode. The PostToolUse path serves as a fallback. To make subagent nodes visible in the parent session's graph, use a module-scoped `Map<string, string>` (`childToParentSession`) that maps child session IDs to their parent session ID. Expand `sessionDeliveries` filtering to include deliveries whose sessionId maps to the selected parent session. Suppress child sessions from the sidebar session list by checking the map. NEVER use `useRef` for this map — it must survive React mount/unmount cycles.
- **Mock vs real event payload mismatch:** `fredo emit` mock events and real opencode agent events have DIFFERENT payload structures. NEVER assume mock payload fields exist in real events. Key differences: user prompt is in `chat.message` `output.message.parts[0].text` (not `UserPromptSubmit` `properties.text`), token counts are in `info.tokens.input/output` (not `turnInputTokens`/`turnOutputTokens`), parent-child relationships exist in TWO real opencode paths — `tool_response.metadata.parentSessionId` (PostToolUse `task` events) AND `properties.info.parentID` (`session.created` events) — both must be checked (spec #461 found `parentID` exists and fires earlier), and `session.next.tool.*` events do NOT exist in real opencode. Always verify extraction paths against the telemetry database (`telemetry_spans` table) via `.opencode/skills/telemetry-query/telemetry-query.ps1` — use real paths with mock fallbacks, not the reverse.
