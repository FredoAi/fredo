...
- **Session splitting chicken-and-egg deadlock (Spec #478):** Populating `childToParentSession` from `session.created` and PostToolUse `task` events creates a rendering-order dependency: `sessionDeliveries` filtering (in `useMemo`/`useDeliverySessions`) consults `getParentSession()` to determine if a child session delivery should be included, but `getParentSession()` can only return a value AFTER the mapping is populated — which happens inside `processDelivery()`, which only processes deliveries in `sessionDeliveries`. Result: child deliveries are excluded from `sessionDeliveries` → mapping never populated → deadlock. **Fix pattern:** Populate the mapping synchronously during render (as side-effects triggered by `deliverySessionId()`/`isChatNodeDelivery()` functions called inside `useMemo`), NOT in `useEffect` (which runs after render). This ensures the map is populated before the sidebar session list filters against it. NEVER try to break the deadlock with `useEffect` + state batching like `mappingVersion` — this just creates race conditions. This is an exception to the normal React rule against side-effects during render; the alternative (polling, retries, version counters) is more fragile.
