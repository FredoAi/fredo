...
- Run `git branch --merged main` and `git for-each-ref --merged main --format='%(refname:short)' refs/heads | Where-Object { $_ -ne 'main' }` periodically to find merged/orphaned branches for cleanup. Branch creation and merge-deletion are state-machine actions (`create-branch`, `merge-pr`); finding stale branches is a read-only hygiene task.
