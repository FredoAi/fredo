...
- **Mock vs real event payload mismatch:** `fredo emit` mock events and real opencode agent events have DIFFERENT payload structures. NEVER assume mock payload fields exist in real events. Key differences: user prompt is in `chat.message` `output.message.parts[0].text` (not `UserPromptSubmit` `properties.text`), token counts are in `info.tokens.input/output` (not `turnInputTokens`/`turnOutputTokens`), parent-child relationships are detected via `session.updated → properties.info.parentID` — PostToolUse `task` events do NOT carry `tool_response.metadata.sessionId` or `parentSessionId` (confirmed via telemetry: `adapter.relationship.detect` = 0 hits, `adapter.session.rewrite` = 7,197 hits), and `session.next.tool.*` events do NOT exist in real opencode. Always verify extraction paths against the telemetry database (`telemetry_spans` table) via `.opencode/skills/telemetry-query/telemetry-query.ps1` — use real paths with mock fallbacks, not the reverse.
