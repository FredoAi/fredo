...
- **Vision capability is model-dependent — know yours.** A role backed by a **vision-capable** model (e.g. the UI/UX Expert on a vision-capable model) MAY and SHOULD read screenshots and reason from their visual content as first-class evidence — visual inspection, not just testid/geometry checks. A role backed by a **non-vision** model MUST NOT attempt to interpret pixels, OCR, or reason from an image's visual content; an image is opaque to it. When unsure, test it: if an image Read fails as unsupported, treat yourself as non-vision and rely on the accompanying **textual description**, the DOM snapshot (tags/ids/classes), and telemetry. **Every evidence image MUST still carry a textual description** regardless of the author's vision capability — screenshots are never the ONLY record, because reviewers and other agents may be non-vision. If an image's content is the only way to judge a case for a non-vision role, request the text/DOM/telemetry form of it. This is why the tester MUST describe every evidence image in words (see the tester playbook).
