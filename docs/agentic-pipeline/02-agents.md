...
> **⚠️ DEPRECATED — scheduled for removal.** The removal condition below is now satisfied: the `.opencode/agents/*.md` files exist and are rewritten to the minimal model (identity + state-call directive, per [08-agent-definition-guide.md](08-agent-definition-guide.md)). This page's content is fully superseded by those files and the playbooks (which hold scope boundaries, guardrails, and mechanics), and the page is **scheduled for removal**. The authoritative source for agent identity is each agent's own file in `.opencode/agents/<name>.md`. Keep this page frozen — do not add or edit profile content here.
