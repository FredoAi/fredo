...
| Self-Improver | Current phase + which work items are `blocked` + staffing Goals + `audit` phase full issue record | Orchestration, staffing, escalation decisions; judging completion; improving prompts/skills/scripts/references; choosing restart phase. **Never researches code or telemetry** (Software Architect's scope) |
