...
**Rounds advance on every rework:** `retry_state`'s round is the count of `phase.started` entries for `testing` (each rework re-enters testing, so round 2, 3, … are stamped correctly — never stuck at "round 1"; the retry *reason* still comes from failed `audit.verdict` events). Round-stamped comments: `## Development Summary (round N)`, `## Tests Runs (round N)`, `## SI Summary (round N)`, `## Fix Plan (round N)`.
