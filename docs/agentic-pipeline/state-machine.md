...
| `audit` | Prints the issue's audit bundle for the Self-Improver AND runs the integrity gate first — a tampered record is flagged before the SI judges. The bundle is the **record-anchored** evidence: recorded history (events, rework, blocked, tester Evidence on the plan, GH record) **plus linked-artifact status** (evidence-on-plan, spec-PR-merged) **plus a telemetry error tail** (ERROR spans, 24h, best-effort) | Issue exists |
