...
| `create-issue` | Creates a **backlog** issue from a drafted body file (the only auto-created issue type — single-issue model). The drafted body is validated against the PO template sections, then posted as-is; **the issue BODY is the single PO Backlog** (no `## PO Backlog` comment is derived or posted — PO feedback #2688). Also adds the issue to the configured GitHub project with Status `Backlog` (best-effort). **The Product Owner's only GitHub write is this action** — `comment`/`post-comments` are gated to reject `product-owner`. (The legacy `--issue-type impl-plan` path is retained for test/back-compat only; the pipeline never creates plan issues.) | Product Owner or Self-Improver; body conforms to PO template sections (backlog/bug); valid issue type |
...
| `generate-work` | **Removed.** Sub-issues, the consolidated tester issue, and separate plan issues were dropped (PO decision): all work is tracked directly on the feature issue + the spec branch, and the tester posts its `## Tests Runs` / `## Evidence` verdict on the feature issue. Calling this action prints a removal note and never creates issues | any actor; no-op |
...
- **`planning → implementation` — plan assembly + test persistence + spec branch.** The machine (a) assembles the plan into the `## Triage Plan` timeline-comment draft `.opencode/tmp/<issue>/triage-plan.md` (from `docs/agentic-pipeline/templates/triage-plan-template.md`, filling every section — `software-architect`, `ui-ux`, `qa`, `summary`, `staffing`, `deployment`, `risks` — from the converged A2A file) and posts it ON the feature issue; (b) persists the QA-seeded test suites to `main` via `tests-commit` (feature names parsed from the QA Expert's `**Feature tests:** <name1, name2>` line in the A2A file); (c) creates the spec branch `spec/<N>`. **Single-issue model: no sub-issues, no tester issue, and no separate plan issue are created** — all work is tracked directly on the feature issue + the spec branch, and the tester posts its `## Tests Runs` / `## Evidence` verdict on the feature issue.
...
**Timeline comments (posted by every transition + `audit-record`):** the feature issue is the single source of truth, and its timeline is built from four titled comments — `## Triage Plan`, `## Development Summary`, `## Tests Runs`, `## SI Summary`. **The issue BODY is the single PO Backlog** — no `## PO Backlog` comment is posted (it duplicated the body; PO feedback #2688). Agents draft each as `.opencode/tmp/<issue>/<file>.md` (`triage-plan.md`, `dev-summary.md`, `tests-runs.md`, `si-summary.md`) using the templates in [templates/](templates/); the state machine reads each pending draft and posts it (consuming the file) as part of the transition / `audit-record`. A `## Tests Runs` draft is **refused unless it carries a literal `Verdict:` line** (a malformed "PASS 7/7" header is kept for the tester to fix). The `## Tests Runs` comment carries the tester's verdict and is read by the verification gate alongside `## Evidence`. A `post-comments` action flushes pending drafts manually.

**Retry-round plan compaction:** the full `## Triage Plan` is posted once at the first `planning → implementation`. On a rework re-entry into implementation (`testing → implementation`, i.e. a prior `phase.started` for `implementation` exists), the machine does NOT re-post the full plan — it posts a compact `## Fix Plan (round N)` derived from the draft: the actionable `### Sub-issue Decomposition` checklist + the `## Risks & Mitigations` section (where the Architect records root-cause + fix scope) + a pointer to the full plan comment. The developer reads the full plan once and the `Fix Plan` deltas on each retry. **A RESCOPE re-entry (`planning → implementation` after an `implementation → planning` rescope) re-posts the FULL re-converged plan** — the scope changed, so the old plan comment is superseded, not "unchanged" (the Fix Plan preamble would be false). The compaction decision keys on the SOURCE phase of the transition: `testing` compacts, `planning` (rescope) re-posts full, and a manual `post-comments` flush keeps the prior-implementation heuristic.
