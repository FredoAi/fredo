...
| `create-issue` | Creates a **backlog** issue from a drafted body file (the only auto-created issue type — single-issue model). The drafted body is validated against the PO template sections, then posted as-is; **the issue BODY is the single PO Backlog** (no `## PO Backlog` comment is derived or posted — PO feedback #2688). Also adds the issue to the configured GitHub project with Status `Backlog` (best-effort). **The Product Owner's GitHub writes are this action plus `Status` comments (PO amendments, #2734)** — `post-comments` remains gated to reject `product-owner`. (The legacy `--issue-type impl-plan` path is retained for test/back-compat only; the pipeline never creates plan issues.) | Product Owner or Self-Improver; body conforms to PO template sections (backlog/bug); valid issue type |
