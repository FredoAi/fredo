...
- **`planning → implementation` — plan assembly + test persistence + spec branch.** The machine (a) assembles the plan into the `## Triage Plan` timeline-comment draft `.opencode/tmp/<issue>/triage-plan.md` (from `docs/agentic-pipeline/templates/triage-plan-template.md`, filling every section — `software-architect`, `ui-ux`, `qa`, `summary`, `staffing`, `deployment`, `risks` — from the converged A2A file) and posts it ON the feature issue; (b) persists the QA-seeded test suites to `main` via `tests-commit` (feature names parsed from the QA Expert's `**Feature tests:** <name1, name2>` line in the A2A file); (c) creates the spec branch `spec/<N>`. **Single-issue model: no sub-issues, no tester issue, and no separate plan issue are created** — all work is tracked directly on the feature issue + the spec branch, and the tester posts its `## Tests Runs` verdict on the feature issue.
