...
**Agreement gate (triage):** leaving triage requires the plan deliverable to be converged — `.opencode/tmp/<issue>/triage.md` must carry all required sections and the `## Convergence: agreed` marker appended by the Self-Improver once no unresolved items remain in the `## Discussion` section. **No GitHub `Decision` convergence comment is posted** — the plan file is the artifact; if an agent looks for a GitHub comment and finds none, it reads the `.md` files under `.opencode/tmp/<issue>/`. The plan does not need to pre-exist as a GitHub artifact: the `triage → implementation` transition assembles it into the `## Triage Plan` comment on the feature issue (from the converged A2A file), persists the test suites, and creates the spec branch. No sub-issues, tester issue, or separate plan issue are generated — the plan's `- [ ]` checklist is the work list.
