...
| `transition` | Moves an issue to the next phase (updates the phase label, and mirrors the phase onto the GitHub project **Status** field — Backlog → Planning → Coding → E2E → Reviewing → Done; best-effort, never fatal). `--to-phase` is optional — inferred when the phase has a single legal exit (required for `testing`/`audit`). **`--to-phase done` is refused** — `done` is reached only through cleanup (`close-issue --to-phase done`, which labels it done and leaves it OPEN for manual human close). **Auto side-effects (idempotent) before the label change:** `backlog → planning` **seeds the A2A working file** `.opencode/tmp/<issue>/triage.md` (from the triage template's per-agent sections + `## Discussion`; on an `audit → planning` restart the stale file is backed up and re-seeded fresh); entering `implementation` **assembles the plan into the `## Triage Plan` timeline draft** `.opencode/tmp/<issue>/triage-plan.md` (from the converged A2A file — no plan issue is created, single-issue model; on a rework re-entry it is posted as a compact `## Fix Plan (round N)` instead), **persists the QA-seeded test suites** (`tests-commit`, feature names parsed from the QA Expert's `**Feature tests:**` line), and creates the spec branch `spec/<N>`; entering `testing` opens the spec PR (`spec/<N>` → `main`); `testing → audit` merges the spec PR (blocked unless it is mergeable). A failed side-effect aborts the transition cleanly — no half-state. **No auto `Status` comment is posted** — the feature issue is the single source of truth, agents comment explicitly; the transition is recorded only in the event log | Self-Improver only; source phase label removed, target label added; legal transition; prior-phase exit guard passes (planning's exit guard checks the plan deliverable: `.opencode/tmp/<issue>/triage.md` converged with all required sections + `## Convergence: agreed`) |
