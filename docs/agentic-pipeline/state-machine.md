...
| `cleanup` | Teardown complete: worktrees removed, stale branches pruned (spec/* kept as evidence), gitignored scratch cleaned, evidence retained, no leftover dirty state | Self-Improver audit verdict = success (`audit-record --verdict success` auto-transitioned `audit → cleanup`; the issue is OPEN with label `cleanup`) | Self-Improver | `done` — the SI runs the teardown then `close-issue --to-phase done` (swaps the label to `done`, records the phase transition, and **leaves the issue OPEN** — the human closes it manually after review) |
