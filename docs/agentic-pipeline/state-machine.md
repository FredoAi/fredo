...
| `comment` | Posts a prefixed comment (`Decision`/`Question`/`Status`/`Evidence`) | Prefix is one of Decision/Question/Status/Evidence; `Decision` is Self-Improver only, `Evidence` is Tester or Self-Improver (both carry exit-guard markers); body-file provided |
| `create-worktree` | Creates a worktree **detached at the tip of the spec integration branch** `spec/<N>` (auto-resolved from the sub-issue's parent plan, mapped to the FEATURE issue (the integration branch is `spec/<feature>`), falling back to `main`). Path defaults to `.worktrees/<N>`. Detached worktrees allow many developers in parallel | Developer only; sub-issue labeled `ready-for-dev`/`in-progress-dev` (single-developer pipeline; no assignee required) |
...
| `close-issue` | Closes an issue to `canceled` (any non-done phase) or `done` (audit-phase features only; **dev sub-issues — body STARTS with `Parent: Implementation Plan #` AND carries a dev label (`ready-for-dev`/`in-progress-dev`) — close as `done` from any phase**, so the Self-Improver closes each sub-issue after reviewing its push). The `done` path for features is normally automatic via `audit-record --verdict success` | Self-Improver only; `--to-phase done\|canceled`; `done` gated to audit-phase features or dev sub-issues; `canceled` refused for done issues |
...
> **Actor identity is self-declared:** the --agent <name> argument is read verbatim and is NOT cryptographically bound to the real caller. The role gates (Decision = self-improver, Evidence = tester|self-improver, udit-record/	ransition = self-improver, etc.) are discipline-level checks within the trusted pipeline-agent pool, not a security boundary - the real enforcement is opencode's permission model, which restricts which agents may invoke the script at all.
...
> **Legacy events:** the event_name enum above is exhaustive for the CURRENT code. Logs from the pre-refactor (scrum-master-era) pipeline - create-spec-branch, create-pr, merge-pr events, outcome: "role", and the scrum-master actor - still exist in older per-issue JSONL files but are never emitted today.
