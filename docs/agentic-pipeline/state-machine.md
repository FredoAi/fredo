...
| **Labels** | The label set (`backlog`, `planning`, `ready-for-dev`, `in-progress-dev`, `ready-for-test`, `testing`, `audit`, `cleanup`, `blocked`, `done`) matches the true phase. Mismatch = the script reports the discrepancy rather than trusting the label. |
| **Templates** | On `create-issue`, the drafted body is validated against the PO template sections (backlog type, incl. bug-variant bodies) — the only template conformance the script enforces. The plan scaffold is `templates/triage-plan-template.md`, assembled by the `planning → implementation` transition into the `## Triage Plan` feature-issue comment. Other bodies are drafted by agents to their templates; the script does not re-validate them. |
| **Comments** | Required comments exist per [github.md](github.md): `Status` (the only agent-facing prefix — blockers/escalations) on the feature issue (the single source of truth), plus the timeline comments (`## Triage Plan`, `## Fix Plan`, `## Development Summary`, `## Tests Runs`, `## SI Summary`) and the machine-only audit `Decision`. The testing exit guard scans the **feature issue** for the tester's `## Tests Runs` verdict — the feature is the single source of truth. No auto `Status` comments are posted — transitions are recorded only in the event log. **The Product Owner never posts a comment directly** (`comment`/`post-comments` are gated to reject `product-owner`); `create-issue` derives the `po-backlog.md` draft from the intake body and the backlog → planning transition auto-posts it as `## PO Backlog`. **Trusted-author comment filter (ST-4 / REQ-05):** on a PUBLIC repo, issue comments are an untrusted prompt-injection surface. Every comment-body read path (`get_issue_comments`, `latest_evidence_comment`, `count_verdict_comments_in_round`, and the audit/cleanup gate + orchestration-snapshot reads) requests `authorAssociation` and EXCLUDES any comment authored by a non-write-capable role (`NONE`/`CONTRIBUTOR`/`FIRST_TIME_CONTRIBUTOR`/`FIRST_TIMER`) — it can never become a `## Tests Runs` verdict, a context-brief input, or the `latest` evidence. Trusted roles = `OWNER`/`MEMBER`/`COLLABORATOR` PLUS the pipeline's own posting principal (`BOT`/`MANNEQUIN`) — excluding those would break verdict parsing and AC3. Each excluded comment emits a `guard.fired` metric event + a surfaced note (never silent). |
...
|--------|------------------------------|---------------------|
| `context` | Prints the phase context block for the dispatched agent (add `--raw` for JSON). For `self-improver` it ALSO prints the **orchestration snapshot** — linked plan #, spec-branch-ahead commit count, Evidence-on-plan, A2A file path, spec branch, open blockers — so the orchestrator sees what exists without re-discovering it | Issue exists |
| `create-issue` | Creates a **backlog** issue from a drafted body file (the only auto-created issue type — single-issue model). The drafted body is validated against the PO template sections, then posted as-is; **the issue BODY is the single PO Backlog** (no `## PO Backlog` comment is derived or posted — PO feedback #2688). Also adds the issue to the configured GitHub project with Status `Backlog` (best-effort). **ST-2 lock-on-create:** immediately after creation the machine locks the new issue's conversation (`lock_reason: off-topic`, reusing the issue number parsed from the create URL) so it is comment-safe at birth — a **best-effort** step (a lock failure is surfaced as a warning + `guard.fired` note, never fatal to creation; the `hardening-lock-open-issues` action reconciles). `temp:` harness issues are NOT locked. **The Product Owner's GitHub writes are this action plus `Status` comments (PO amendments, #2734)** — `post-comments` remains gated to reject `product-owner`. (The legacy `--issue-type impl-plan` path is retained for test/back-compat only; the pipeline never creates plan issues.) | Product Owner or Self-Improver; body conforms to PO template sections (backlog/bug); valid issue type |
...
| `update-plan` | Replaces one whole `##` section of the `## Triage Plan` timeline draft `.opencode/tmp/<issue>/triage-plan.md` (the plan the `planning → implementation` transition auto-posts on the feature issue): `--issue <feature> --section <key> --body-file <draft>` replaces that `##` section (idempotent — re-running overwrites only that section; all others are untouched). Sections: `software-architect`, `ui-ux`, `qa` (agent sections) and `summary`, `staffing`, `deployment`, `risks` (SI sections). **The `planning → implementation` transition assembles the plan and fills all sections automatically** — the action is kept for edge/repair only | Self-Improver only; `--section` (one of `software-architect`/`ui-ux`/`qa`/`summary`/`staffing`/`deployment`/`risks`) + `--body-file` required |
| `transition` | Moves an issue to the next phase (updates the phase label, and mirrors the phase onto the GitHub project **Status** field — Backlog → Planning → Coding → E2E → Reviewing → Done; best-effort, never fatal). `--to-phase` is optional — inferred when the phase has a single legal exit (required for `implementation`/`testing`/`audit`). **`--to-phase done` is refused** — `done` is reached only through cleanup (`close-issue --to-phase done`, which labels it done and leaves it OPEN for manual human close). **Human-authorized pre-verdict rework (`--human-authorized`):** a binding human directive may add scope while an issue sits in `testing` with its round aborted before any tester verdict (so no `## Tests Runs` comment can exist yet, and the normal rework leg's evidence precondition cannot be met). Passing `--human-authorized` together with a non-empty `--reason` on `transition --to-phase implementation` bypasses ONLY that leg's evidence exit guard and records an auditable `human.authorization` event; the flag is ignored on every other leg, and without a non-empty reason the guard still blocks. **REOPEN leg (`done → planning`):** a post-merge defect reported during human review (the issue is still OPEN, labeled done) loops the feature back to planning so the triage cluster re-converges a fix round — the stale converged A2A is backed up and re-seeded fresh (same as the restart/rescope legs) and the retry round advances from the testing-entry count (machine-stamped); `reopenCount`/`reopenRate` derive from these transition events. **Auto side-effects (idempotent) before the label change:** `backlog → planning` **seeds the A2A working file** `.opencode/tmp/<issue>/triage.md` (from the triage template's per-agent sections + `## Discussion`; on an `audit → planning` restart, an `implementation → planning` **rescope**, or a `done → planning` **reopen** the stale file is backed up and re-seeded fresh); entering `implementation` **assembles the plan into the `## Triage Plan` timeline draft** `.opencode/tmp/<issue>/triage-plan.md` (from the converged A2A file — no plan issue is created, single-issue model; on a rework re-entry the **Software Architect-authored** `fix-plan.md` is posted as a compact `## Fix Plan (round N)` instead, falling back to a derived compaction of the plan draft when no authored fix-plan exists; on a **rescope re-entry (`planning → implementation`) the FULL re-converged plan is re-posted** — the old plan comment is superseded, not "unchanged"), **persists the QA-seeded test suites** (`tests-commit`, feature names parsed from the QA Expert's `**Feature tests:**` line), and creates the spec branch `spec/<N>`; entering `testing` opens the spec PR (`spec/<N>` → `main`); `testing → audit` merges the spec PR (blocked unless it is mergeable — **note: GitHub CI checks are DEACTIVATED until open-source** (`.github/workflows/validate.yml.disabled`, human decision 2026-08-25), so `statusCheckRollup` is empty and `pr_merge_guard` passes on merge state alone; re-enable checks by restoring the workflow filename). A failed side-effect aborts the transition cleanly — no half-state. **No auto `Status` comment is posted** — the feature issue is the single source of truth, agents comment explicitly; the transition is recorded only in the event log | Self-Improver only; source phase label removed, target label added; legal transition; prior-phase exit guard passes (planning's exit guard checks the plan deliverable: `.opencode/tmp/<issue>/triage.md` converged with all required sections + `## Convergence: agreed`; implementation's forward commit guard is bypassed for the backward rescope leg) |
| `close-issue` | Closes the issue as `done` (from the **cleanup** phase — teardown complete; the audit verdict was recorded at `audit-record --verdict success`) or `canceled` (from any non-done phase). `--to-phase done` swaps the label `cleanup → done`, records the phase transition, posts the final-metrics summary, and closes the issue; `--to-phase canceled` closes without a phase swap | Self-Improver only; `--to-phase done|canceled` required; `done` allowed only from cleanup (fail-closed on the same verification audit-record enforces) |
| `comment` | Posts a prefixed comment (`Status` only — blockers/escalations; `Evidence`/`Question`/free-form `Decision` are removed and refused with a pointer to the correct channel) from a draft the agent wrote as `.opencode/tmp/<issue>/<prefix>.md` (using the comment templates in [templates/](templates/)); `--body-file` is optional and overrides the conventional path. The machine validates the body (Evidence must carry a verdict) then prefixes `## <Prefix>` and posts | Prefix is one of Decision/Question/Status/Evidence; `Decision` is Self-Improver only, `Evidence` is Tester or Self-Improver (both carry exit-guard markers); Evidence body must contain a verdict; **`product-owner` is rejected** (the PO's only output is deterministic — see `create-issue`) |
| `create-worktree` | Creates a worktree **detached at the tip of the spec integration branch** `spec/<N>` (the developer works directly on the feature issue — sub-issues were removed; the branch is `spec/<feature>`, falling back to `main` when the issue is not a feature). Path defaults to `.worktrees/<N>`. Detached worktrees allow many developers in parallel | Developer only; issue labeled `ready-for-dev`/`in-progress-dev`/`ready-for-test`/`testing` (single-developer pipeline; no assignee required). `testing` is accepted for the CI-fix path (G-164): a check that goes red on the open spec PR is fixed by a scoped commit on the spec branch while the feature is still in `testing`, so the developer must be able to obtain a worktree then |
...
| `generate-work` | **Removed.** Sub-issues, the consolidated tester issue, and separate plan issues were dropped (PO decision): all work is tracked directly on the feature issue + the spec branch, and the tester posts its `## Tests Runs` verdict on the feature issue. Calling this action prints a removal note and never creates issues | any actor; no-op |
| `prune` | Branch + worktree hygiene: deletes local `spec/*`/`feat/*` branches whose remote counterpart is **gone** (deleted upstream), legacy merged `feat/*`, orphaned worktrees + unregistered `.worktrees/*` debris; with `--remote` also deletes `origin/spec/<N>` for every **CLOSED** spec (open-issue list read once; fail-safe — a failed/unparseable read skips the sweep) | Self-Improver only; idempotent; never `main`/`master`/other. Merged spec PRs auto-delete their head branch via the repo's `delete_branch_on_merge` |
| `set-permission` | Grants/revokes ONE rule in `opencode.json` for ONE agent: `--role <agent> --tool <category> --pattern "<glob>" --decision allow\|ask\|deny`. Edits **agent `permission` blocks only** — the rest of `opencode.json` and `AGENTS.md` stay human-owned. Text-surgical and order-preserving (opencode evaluates permission rules last-match-wins, so the rule is APPENDED and the document is re-parsed before writing). Idempotent (a no-op when the rule already holds the value); refuses an unknown role/category, an invalid decision, and a flat-action category. `--config-file` overrides the target path (tests/bootstrap). No `--issue` required; takes effect on the next opencode restart | Self-Improver only |
| `upload-evidence` | Uploads a screenshot to GitHub as a `user-attachments` asset via the `gh-image` extension (`gh image`) and prints the `https://github.com/user-attachments/...` URL for the tester to embed in the `## Tests Runs` comment — nothing is committed to the repo, so evidence never bloats `main` or the spec branch | Tester or self-improver; `--image` required; `--body-file`/`--base` accepted for backward-compat but unused; requires the `gh-image` extension |
| `close-issue` | Closes an issue to `canceled` (any non-done phase) or `done` (**cleanup phase only** — teardown complete; the audit verdict was recorded at `audit-record --verdict success`, which auto-moved audit → cleanup). The issue is labeled done and left OPEN — the human closes it manually after review | Self-Improver only; `--to-phase done\|canceled`; `done` gated to the cleanup phase (rs: `if phase != Phase::Cleanup { block }`); `canceled` refused for done issues |
| `block` / `unblock` | Sets/clears the `blocked` modifier with reason | Self-Improver or Developer; reason present (`block`); label toggled |
| `audit` | Prints the issue's audit bundle for the Self-Improver AND runs the integrity gate first — a tampered record is flagged before the SI judges. The bundle is the **record-anchored** evidence: recorded history (events, rework, blocked, tester Evidence on the plan, GH record) **plus linked-artifact status** (evidence-on-plan, spec-PR-merged). **The SI never queries telemetry to audit** — code/telemetry research belongs to the Software Architect | Issue exists |
...
- **`backlog → planning` — A2A seed.** The machine auto-seeds the A2A working file `.opencode/tmp/<issue>/triage.md` (idempotent). The Self-Improver does NOT run `triage-init` manually.
- **`planning → implementation` — plan assembly + test persistence + spec branch.** The machine (a) assembles the plan into the `## Triage Plan` timeline-comment draft `.opencode/tmp/<issue>/triage-plan.md` (from `docs/agentic-pipeline/templates/triage-plan-template.md`, filling every section — `software-architect`, `ui-ux`, `qa`, `summary`, `staffing`, `deployment`, `risks` — from the converged A2A file) and posts it ON the feature issue; (b) persists the QA-seeded test suites to `main` via `tests-commit` (feature names parsed from the QA Expert's `**Feature tests:** <name1, name2>` line in the A2A file — matched leniently, so a leading blockquote `>` / whitespace before the marker does not silently drop the declaration, **but the tokens AFTER the marker must themselves be bare comma-separated lowercase-kebab names**: a marker embedded in a comma-rich sentence parses to ZERO names, and the transition now surfaces a `WARNING:` note + a `guard.fired` (G-240) instead of skipping silently) — **only on the FIRST entry into implementation**; a rework (`testing → implementation`) or rescope (`implementation → planning → implementation`) re-entry skips suite persistence, because the root checkout may sit on a spec branch whose `.opencode/tests/**` copies predate the tester's later suite extensions and re-persisting from that stale tree would revert main's guidance (the G-038 hazard); (c) creates the spec branch `spec/<N>`. **Single-issue model: no sub-issues, no tester issue, and no separate plan issue are created** — all work is tracked directly on the feature issue + the spec branch, and the tester posts its `## Tests Runs` verdict on the feature issue.

The planning exit guard is now **only** the plan deliverable — `.opencode/tmp/<issue>/triage.md` must be converged (all required sections + `## Convergence: agreed`); no GitHub `Decision` convergence comment is posted. If an agent looks for a GitHub comment and finds none, it reads the `.md` files under `.opencode/tmp/<issue>/`. The manual `triage-init`, `tests-commit`, and `update-plan` actions are kept for edge/repair only; the transition owns the happy path. The `po-backlog.md` draft is derived by `create-issue` (the PO never writes a comment directly).

**Timeline comments (posted by every transition + `audit-record`):** the feature issue is the single source of truth, and its timeline is built from five titled comments — `## Triage Plan`, `## Fix Plan (round N)` (rework re-entries), `## Development Summary`, `## Tests Runs`, `## SI Summary`. **The issue BODY is the single PO Backlog** — no `## PO Backlog` comment is posted (it duplicated the body; PO feedback #2688). Agents draft each as `.opencode/tmp/<issue>/<file>.md` (`triage-plan.md`, `fix-plan.md`, `dev-summary.md`, `tests-runs.md`, `si-summary.md`) using the templates in [templates/](templates/); the state machine reads each pending draft and posts it (consuming the file) as part of the transition / `audit-record`. A `## Tests Runs` draft is **refused unless it carries a literal `Verdict:` line** (a malformed "PASS 7/7" header is kept for the tester to fix), **and refused if any image reference lacks `https://`** — a bare screenshot filename or local `.opencode/tmp/` path is unviewable/unopenable on GitHub (observed #2756 round 2), so the machine keeps the draft until every image is an `upload-evidence` raw URL. The `## Tests Runs` comment carries the tester's verdict and is the ONLY comment the verification gate reads for evidence. A `post-comments` action flushes pending drafts manually.

**Retry-round plan compaction:** the full `## Triage Plan` is posted once at the first `planning → implementation`. On a rework re-entry into implementation (`testing → implementation`, i.e. a prior `phase.started` for `implementation` exists), the machine does NOT re-post the full plan. The fix scope is designed by the **Software Architect**: the SI dispatches the architect (with the tester's FAIL verdict) BEFORE the transition; the architect writes `.opencode/tmp/<issue>/fix-plan.md` (`Root cause class: defect|technique|environment|scope` as the first content line, then `## Failed ACs`, `## Root Cause (file:line)`, `## Fix Scope`, ending `*Authored by Software Architect*`), and the machine posts it as the compact, round-stamped `## Fix Plan (round N)` — consuming both drafts so nothing double-posts. **The class line is mandatory**: the machine records it as a `rework.rootcause` event attribute (the per-round WHY the SI trends — most rounds burn in this loop, not at audit) and refuses to post an unclassified draft. **Fallback only:** when no authored draft exists, the machine derives a compact plan from the draft — the actionable `### Sub-issue Decomposition` checklist + the `## Risks & Mitigations` section + a pointer to the full plan comment; the developer reads the full plan once and the `Fix Plan` deltas on each retry. The orchestrator never authors fix scope itself. **A RESCOPE re-entry (`planning → implementation` after an `implementation → planning` rescope) re-posts the FULL re-converged plan** — the scope changed, so the old plan comment is superseded, not "unchanged" (the Fix Plan preamble would be false). The compaction decision keys on the SOURCE phase of the transition: `testing` compacts, `planning` (rescope) re-posts full, and a manual `post-comments` flush keeps the prior-implementation heuristic. A stray `fix-plan.md` on a FIRST entry is never posted (kept on disk with a warning).

**Rounds advance on every rework:** `retry_state`'s round is the count of `phase.started` entries for `testing`, advanced EARLY while a rework is in flight — an implementation re-entry from `testing` (tester-FAIL rework) or `audit` (restart) also counts, so an agent dispatched ONTO the rework (before the next testing entry lands) sees the correct round and the RETRY marker, never "round 1"; the retry *reason* still comes from failed `audit.verdict` events. Round-stamped comments: `## Development Summary (round N)`, `## Tests Runs (round N)`, `## SI Summary (round N)`, `## Fix Plan (round N)`.

**Project Status sync (labels + Status):** alongside the phase labels, every transition mirrors the new phase onto the configured GitHub project's **Status** single-select field (`Backlog → Planning → Coding → E2E → Reviewing → Done`; `create-issue` adds the issue to the project, `close-issue done/canceled` sets `Done`). The issue is added on first sync; on `done` it stays OPEN (manual human close) with Status = `Done`. Best-effort and fail-safe — a sync failure logs `[project-status]` and is never fatal.
...
**Metric anchors (fixed by policy):** commitment point = the Backlog→Planning handoff (lead-time clock starts); delivery point = Cleanup→Done (cycle-time clock ends). Cycle time = Implementation entry → Done; lead time = commitment → Done. Backlog/Planning count toward lead time only. See [Metrics](#metrics-the-pipelines-memory).

| Phase | Goals (definition of done) | Entry condition (signal) | Owner | Exits to |
|-------|----------------------------|--------------------------|-------|----------|
| `backlog` | Backlog issue exists with the required intake sections (## Title, ## Problem / Why now, ## Intended users, ## Proposed behavior / Scope, ## Success metrics, ## Acceptance criteria, ## Out of scope, ## Priority), label `backlog` | Backlog issue exists, label `backlog`, required sections present (the machine does not separately enforce "no Implementation Plan" — a plan existing mid-backlog is a non-issue) | Product Owner | `planning` |
| `planning` | Deliberation converged: each planner wrote its section draft and agent-tagged points in the A2A working file `.opencode/tmp/<issue>/triage.md`, no unresolved `## Discussion` items remain, and `## Convergence: agreed` is appended to the file. **The implementation plan (the A2A file) is the planning deliverable** — no GitHub `Decision` convergence comment exists. The plan is auto-assembled by the `planning → implementation` transition — it does not need to pre-exist | Feature labeled `planning` (transition from `backlog`; the A2A file is auto-seeded) | Self-Improver (orchestrator; machine phase owner per `pipeline.json`) | `implementation` — **exit guard checks the plan deliverable: all required sections + `## Convergence: agreed` in the A2A file (no comment required)** |
| `implementation` | Spec integration branch `spec/<N>` exists and has commits beyond `main` (the developer pushed — **commits ahead of main is the exit signal**) | Implementation Plan present (assembled by the transition); machine phase owner is `developer` (per `pipeline.json`) | Self-Improver (setup/review) + Developer (execution) | `testing` — **exit guard: spec branch has commits beyond main**; **`planning` (backward rescope — the "loop back to Phase 2 (Architect)" scope-redesign rule; the forward commit gate does NOT apply: the human-authorized rescope re-seeds the A2A fresh and the planning cluster re-converges the new scope)** |
| `testing` | Tester verdict posted with per-case evidence; failures re-dispatched to the plan's checklist | Feature labeled `ready-for-test` | Tester | `audit` or back to `implementation` — **exit guard: the verification guardrail.** The verdict must substantiate a PASS under the plan's `> Verification policy`: for a `live` policy the evidence must reference `telemetry_spans` (a live-query result); a static-only PASS, a FAIL verdict, or an absent `## Tests Runs` comment all block the exit. `audit-record --verdict success` and the `done` close path re-check the same verification and fail CLOSED. **Exception:** a binding human directive that adds scope to a round aborted before any verdict may leave `testing` for `implementation` via `transition --to-phase implementation --human-authorized --reason <why>` (recorded as a `human.authorization` event) — the dev work must then precede testing |
| `audit` | Self-Improver verdict posted: success, or a restart phase + applied improvement | Tester Evidence verdict present (the machine requires a verdict comment, not necessarily "all pass" — "all pass" is the Self-Improver's judgment, since a FAIL verdict routes back to implementation) | Self-Improver | `cleanup` (success) or restart to `backlog`/`planning`/`implementation`/`testing` — `audit-record --verdict success` auto-transitions `audit → cleanup`; a restart auto-transitions `audit → <p>` |
| `cleanup` | Teardown complete: worktrees removed, stale branches pruned (`prune --remote` — spec branches for closed specs deleted), gitignored scratch cleaned, evidence lives on GitHub as `user-attachments`, no leftover dirty state | Self-Improver audit verdict = success (`audit-record --verdict success` auto-transitioned `audit → cleanup`; the issue is OPEN with label `cleanup`) | Self-Improver | `done` — the SI runs the teardown then `close-issue --to-phase done` (swaps the label to `done`, records the phase transition, and **leaves the issue OPEN** — the human closes it manually after review) |
| `done` | Feature labeled `done`, branches cleaned, human review + close | Cleanup complete (issue labeled `done` from cleanup, left OPEN for human close) | Self-Improver + human review | `planning` — **reopen leg**: a post-merge defect reported during human review (while the issue is still OPEN) loops the feature back to planning for a fix round (A2A re-seeded fresh, round machine-stamped); otherwise terminal — the human closes the issue manually |
...
**Agreement gate (planning):** leaving planning requires the plan deliverable to be converged — `.opencode/tmp/<issue>/triage.md` must carry all required sections and the `## Convergence: agreed` marker appended by the Self-Improver once no unresolved items remain in the `## Discussion` section. **No GitHub `Decision` convergence comment is posted** — the plan file is the artifact; if an agent looks for a GitHub comment and finds none, it reads the `.md` files under `.opencode/tmp/<issue>/`. The plan does not need to pre-exist as a GitHub artifact: the `planning → implementation` transition assembles it into the `## Triage Plan` comment on the feature issue (from the converged A2A file), persists the test suites, and creates the spec branch. No sub-issues, tester issue, or separate plan issue are generated — the plan's `- [ ]` checklist is the work list.
...
```text
=== PIPELINE STATE ===
Phase:            <backlog|planning|implementation|testing|audit|cleanup|done>
...
Evidence on plan:   <true/false — tester posted ## Tests Runs>
...
|--------|--------|------------|--------|
| `leadTime` | ▫️ designed | doneAt − committedAt | **commitment point = Backlog→Planning handoff** (fixed by policy) |
| `cycleTime` | ▫️ designed | doneAt − startedAt | **delivery point = Cleanup→Done**; cycle starts at Implementation entry |
...
| `reopenCount` | ✅ implemented | done → reopened (`done → planning` reopen transition events) | events |
...
| `rootCauseMix` / `dominantRootCauseClass` | ✅ implemented | the recorded failure classes (`defect\|technique\|environment\|scope`) — per-round from `rework.rootcause`, per-restart from `audit.verdict` `rootCause`; the dominant class names the **lever** the SI must strengthen (Class → Lever, `playbooks/self-improver.md`) |
...
| `reopenRate` | ▫️ designed | audit-gate's own error rate (`reopenCount` events ÷ completed features) |
...
|-------|-------|----------|
| Product Owner | `backlog` phase + backlog Goals | Knowing the definition of done for intake (confirmed requirements, ACs, priority) |
| Self-Improver | Current phase + which work items are `blocked` + staffing Goals + `audit` phase full issue record | Orchestration, staffing, escalation decisions; judging completion; improving playbooks/skills/scripts/references; choosing restart phase. **Never researches code or telemetry** (Software Architect's scope) |
