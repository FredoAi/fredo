...
|---------|------|-----------|----------|
| **Per-conversation lock** | durable | `POST /repos/{owner}/{repo}/issues/{n}/lock` with `lock_reason: off_topic` — a locked issue is commentable only by users with write access | permanent (until unlocked); the one-shot `hardening-lock-open-issues` locks every currently-OPEN pipeline issue; `create-issue` locks a new issue at birth |
...
| Hardening: lock all OPEN pipeline issues | `hardening-lock-open-issues` (Self-Improver) — enumerate OPEN pipeline issues (pipeline label) and `POST .../issues/<n>/lock` with `lock_reason: off_topic`; skips `temp:` harness issues; one-shot |
