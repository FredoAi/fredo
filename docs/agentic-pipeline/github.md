...
- **Worktrees are created by the state machine** via the `create-worktree` action once the feature is in the implementation phase (labeled `ready-for-dev`); the base is the feature's own `spec/<N>` branch. The `testing` label is also accepted (G-164) so a scoped CI fix can land on the open spec branch while the feature is in `testing`. `remove-worktree` cleans up; `prune` removes orphaned worktrees.
