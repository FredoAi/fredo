...
- **effectiveness:** Partial (2026-09-06, #2830 audit) — the `line_is_telemetry_negation` guard did NOT fully exclude this run's negating phrasing: the pure-rendering plan's QA note "**There is NO `telemetry_spans` leg**" and "do NOT invent one" was reflected in the audit's `live_telemetry_evidence: true` field (the only `telemetry_spans` mention in the plan). `verification_ok` still passed (via G-108's rendered-webview path, not the telemetry scan), so no false block occurred — but the informational `live_telemetry_evidence` field is still counting a negation mention, so the guard's phrasing coverage is incomplete. (Prior: Confirmed 2026-09-03, #2798 audit — hardened the parser + live-evidence scan in pipeline-state.rs, harness 97/97.)
