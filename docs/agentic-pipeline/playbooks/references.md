...
- **effectiveness:** **Partial** (re-validated 2026-09-05, #2810) — the brief gap recurred a third time: the #2810 planning brief instructed the QA Expert to "declare `**Feature tests:** <feature-name>`" but did NOT tell it to author the four `.opencode/tests/window-engine-cleanup/*.md` files, so the transition's `tests-commit` no-op'd and the SI had to seed the suite before the tester entry. The G-098 fallback (SI seeds) worked, but the SI's planning-cluster brief is still not naming the four files explicitly. (Prior: #2807 si re-dispatch to seed; #2797 brief lacked the instruction.) The brief MUST name the four files for the QA Expert to author in the same pass.
