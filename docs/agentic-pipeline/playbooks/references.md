...
- **effectiveness:** Pending (re-validated 2026-09-03, #2803 — applied correctly for a repo-config spec whose ACs require repo-ADMIN enforcement verbs, the sibling case to the timing-verb case it was born from: the QA Plan authored test cases against the allowed in-repo surface (declarative workflow/CODEOWNERS/doc reads), the admin-enforced/publish legs were marked `n/a — owner-gated (environment/tool-access; documented-partial)` and routed to the PO, and NO fake-verify and NO defect mis-classification occurred. <2 specs since activation, so Pending per the effectiveness recipe.)
...
- **effectiveness:** **Partial** (updated 2026-09-05, #2807) — the brief gap recurred: the #2807 planning brief told the QA Expert to "seed the suite name to use" (a `**Feature tests:**` line) but did NOT instruct it to author the four `.opencode/tests/window-manager/*.md` files, so `tests-commit` at the transition had no files to persist and the suite was absent until the SI re-dispatched the QA Expert to seed it in the implementation phase. The G-098 fallback (SI re-dispatch to seed) worked, but the brief must be tightened: name the four files explicitly in the QA Expert brief so the suite is present at the `planning → implementation` transition.
