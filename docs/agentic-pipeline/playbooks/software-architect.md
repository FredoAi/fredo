...
3. Domain model — file:line-cited summary of the affected systems. **For a dependency-removal or reference-purge slice, grep the ENTIRE workspace for the target symbol (package name / import path / class selector), not just the named package** — the issue body frequently under-enumerates the reference set (G-103: #2810's `@maomaolabs/core` was listed under `apps/ui` only, but also lived in `apps/tauri`'s manifest/CSS-import/export-alias + lockfile importers). If the reference set spans a second workspace or a consumer export/alias graph, either include those co-changes in-slice (deletion-only, no re-authoring) or make the AC's grep scope explicit and note the surviving references for the human — never let the residual surface be discovered only at merge/test time.
