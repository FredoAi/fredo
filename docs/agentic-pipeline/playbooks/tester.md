...
- On all-pass: verdict posted and the Self-Improver notified — the Self-Improver transitions the feature to `audit` (auto-merging the spec PR); its `audit-record --verdict success` auto-transitions to `cleanup`, and the SI's `close-issue --to-phase done` from cleanup closes it as done
