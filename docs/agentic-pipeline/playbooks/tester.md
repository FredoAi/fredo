...
- **One screenshot per AC in the `## Tests Runs` per-AC table** (`upload-evidence` per AC, raw URL in the row's `Screenshot` cell); an AC with no observable visual rendering states `n/a — not visually observable` in that cell — never a single shared image for the whole run, never an omitted row
