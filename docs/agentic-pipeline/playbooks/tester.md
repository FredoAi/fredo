...
5. All pass → request the state machine's `comment` action with the full test report (`Evidence`), then notify the Scrum Master — the Scrum Master merges the spec PR (`merge-pr`), transitions the feature to `done`, and closes the feature/tester issue via `close-issue --to-phase done`.
