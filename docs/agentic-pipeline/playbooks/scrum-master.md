...
2. **Dispatch the triage cluster in parallel** (software-architect, ui-ux-expert, qa-expert) with the backlog as the brief; wait for all three planners' section drafts (each posted as a `Decision` comment on the feature issue).
3. **Coordinate the deliberation** — the planners cross-review each other's drafts on the feature issue: they post `Question` comments for conflicts/gaps and the owning planner resolves each with a `Decision` reply. Track open questions; route each to its owning section; never let a `Question` orphan.
4. **Post the convergence marker** — when no planner question remains open, request the `comment` action with a `Decision` comment, body `Triage converged — all planner questions resolved.` This marker is the **agreement gate**: the state machine refuses `triage → implementation` without it.
5. **Create the Implementation Plan from the template** — request `create-issue --issue-type impl-plan` with **no** `--body-file` (the machine seeds the body from `docs/agentic-pipeline/templates/triage-plan-template.md`, filling the `<issue>`/`<title>`/`<backlog>` placeholders), then write each agent's agreed section into the seeded issue via `update-plan --issue <impl-plan-N> --section <agent-or-key> --body-file <draft>` (idempotent per-section replacement). Post a `Status` comment via the state machine's `comment` action.
6. **Staff** using the heuristic `ceil(total points / 5)`, capped by pool capacity at max 2 active sub-issues per developer.
7. **Generate work items via the state machine** — request the `generate-work` action on the Implementation Plan issue; it creates the dev sub-issues (from the plan's `- [ ]` items in the Software Architect's `### Sub-issue Decomposition` section, label `ready-for-dev`) and the consolidated tester issue (from the QA Expert's `### QA Plan`, label `testing`).
8. **Transition triage → implementation** — request the `transition` action; the spec integration branch `spec/<N>` is auto-created as a side-effect. All sub-issue work and testing happens on it; no action needed.
9. **Dispatch developers**; track dependencies; queue work when the pool is saturated. Developers run in parallel, each in a worktree detached at `spec/<N>`, pushing with `HEAD:spec/<N>`.
10. **Review each dev's pushes** on `spec/<N>` against their sub-issue (scope respected, verification comment matches); return failed work to the same developer with a focused change list.
11. **Transition the feature to `testing`** when all sub-issues are pushed to `spec/<N>` — this applies the `testing` label and auto-creates the spec PR; **dispatch the tester** on the consolidated tester issue.
12. On tester pass, transition `testing → audit` — this auto-merges the spec PR (the branch survives so evidence URLs keep rendering); then **dispatch the self-improver** with the issue's full record.
13. On the SI's success verdict, the feature is **already done and closed** — `audit-record --verdict success` auto-transitions `audit → done` and closes as done. Post a final `Status` summary, keep `spec/<N>`, initiate human review. On restart, `audit-record --verdict restart --phase <p>` already re-labeled the issue — **re-dispatch from the chosen phase with the improvement context**.
14. **Handle blockers** — request the `block` action on `blocked` sub-issues; intervene within the 4h SLA; route underspecified sub-issues back to triage; escalate >3 PR rejections to the human with what was tried.
...
- Triage deliberated and converged: all three planner drafts posted as `Decision` comments, every planner `Question` resolved by a `Decision` reply, and the convergence marker posted on the feature issue before the plan was created.
