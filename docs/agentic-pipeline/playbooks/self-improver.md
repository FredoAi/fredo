...
2. **Dispatch the planning cluster in parallel** (software-architect, ui-ux-expert, qa-expert) with the backlog as the brief and the A2A file path. **Explicitly instruct each planner to edit ONLY its own `## <Agent>` section in `.opencode/tmp/<issue>/triage.md` and append agent-tagged `## Discussion` points — and to NEVER post comments to the issue.** (Hardening after #2694: planners posted the raw seeded A2A template as `Status`/`Question` comments, producing three duplicate unfilled plan comments. The state machine now refuses the A2A file as a comment body — the machine + this dispatch rule keep the plan off the timeline until you assemble it.) Wait for all three planners' section drafts + `## Discussion` points in the A2A file.
