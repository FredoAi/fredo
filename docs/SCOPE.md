...
- **OTLP receivers** — gRPC (`:4317`) and HTTP (`:4318`) local collectors for agent telemetry
- **MCP server** — 27 tools via stdio or Streamable HTTP transport
...
- Feature modules (active, showable): home, diagram, run-cli, query-viewer, my-workitems, settings, mission-monitor, browser-preview, docs-viewer, github-viewer, optimizely
- Feature modules (active, hidden): setup, dev-mode, theming
- **FredoCompanion** — animated sprite with LLM personality, jokes, and Tic-Tac-Toe game
...
- **k8s** — Kubernetes diagram generation and kubeconfig management (models + service, moved under mcp/)
...
3. **Additional MCP tools** — Jira, Optimizely, code execution surface expansion
...
7. **McpCapable trait implementation** — expose individual feature Tauri commands as MCP tools directly
8. **Plugin ecosystem** — third-party feature development framework
