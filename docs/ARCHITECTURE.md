...
- **Layout toggle (Spec #2752)**: a small floating segmented control (`LayoutModeToggle`, `data-testid="mm-layout-toggle"`) overlays the canvas (`mm-canvas-wrapper`, top-left, outside the SessionTokenBar whose pure-prop contract is untouched) and switches between **Chain** (default, byte-identical deterministic layout above) and **Force** — a LIVE animated d3-force simulation (`createLiveForceSimulation` in `lib/layout.ts`, rAF-driven, `freezeOnSettled` whole-sim stop at alpha < alphaMin, `forceY` parameterized). In Force mode the chain overrides are skipped so ALL node types (chat/tools/subagent) participate; structural changes restart the simulation seeded from current positions (no jump-to-zero); mode switches preserve node data and the viewport (no auto-fit); Force→Chain restore is an instant snap to the deterministic chain. The selected mode persists across panel close/reopen via `usePersistedSetting` under `Fredo_mm_layout_mode` (the shared SQLite-backed settings path — no `localStorage` in feature source). `prefers-reduced-motion` converges the Force layout synchronously (no rAF glide).
