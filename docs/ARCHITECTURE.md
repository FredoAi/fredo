...
- **Data source**: `StreamContext.deliveries` (append-only `ContractDelivery[]`) via the `chat-node` ECE contract — `streamFields: ['payload', 'state']`, `transports: ['otlp_grpc']` (as of Spec #615, Mission Monitor subscribes to OTLP gRPC exclusively — Hook and OTLP HTTP transports removed to prevent duplicate nodes from dual-transport events), `eventTypes: ['chat']` (Spec #2688 — `agent_session` removed so session spans can never create chat-node buffers or phantom nodes), composite key `(sessionId, correlationId)`, `completeWhen: "state === 'Response'"`. Only the `chat-node` contract is active — `tool-use-lifecycle` and `custom-event` contracts remain deactivated since #593.
...
- **Session token bottom bar** (Spec #2717): a third flex sibling below the canvas body (never an overlay) shows the **selected session's** total token usage as five labeled figures — Input / Cache / Reasoning / Output / Total (Total = Input + Cache + Reasoning + Output exactly; Cache = `cacheReadTokens` only, cache write carried but never summed). Hidden when no session is selected. The same five-way breakdown renders on every chat node and in the DetailPanel. Session totals are derived from `mergedDeliveries` via `computeSessionTokenTotals()` (per-composite-key last-wins dedupe — Σ per-node == session figure by construction; composited child-session deliveries excluded).
