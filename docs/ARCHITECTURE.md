...
- **Command-bar chat (#2871)**: while the companion is active (present at the home seat — `isVisible && !isAway`), the launcher command bar doubles as a chat input. Enter is **smart**: a query that exactly matches an app/feature tile name (trimmed, case-insensitive, full-name) launches as before; any other non-empty query is sent to the companion LLM as a **single-shot** message (no transcript/memory) and the streamed reply renders in the companion's existing `SpeechBubble`. The bar shows a busy state (`aria-busy`, read-only input, `Fredo is replying…` hint) for the whole stream, and Enter is a no-op while generating (never launches a tile or starts a second stream). Dispatch goes through the per-window `CompanionEntity` registry (`askActiveCompanion`), reusing the entity's shared generation core and single-in-flight/stale-token guard; `llm-error` is routed through a typed `onError` channel and mapped to a readable sentence (never the raw backend string). A bar message uses a general chat persona; the single-click joke path keeps its joke persona. With the companion OFF or away, the bar is filter/launch only. Reduced motion keeps the streaming cursor static.
- **Dictation into the bar and Fredo chat (#2878)**: the #2877 voice session's finalized transcript routes into the existing text surface — the launcher command bar for a `launcher`-origin session, and the companion listening bubble for a `companion`-origin session. While listening, live partials render into the bar (the text stays editable; a manual edit stops partial writes, finals still append). On finalize the transcript becomes the bar's query and the SAME smart-Enter contract runs through a single shared commit path — exact tile name launches, otherwise the active companion receives it via `askActiveCompanion`; launch wins; an inactive companion keeps launch/filter; Enter is a no-op while replying. The finalize commit's evidence is the session's own recognized FINAL transcript (the session-scoped delta of the append-only committed accumulator), never the bar's text mirror — so a silent session, a partial-only session, a backend-initiated cancel, or a typed-error end restores the pre-session text once and never dispatches (the pre-fix phantom-dispatch defect and its discriminator tests are pinned in the launcher suite). **Send voice transcripts automatically** (Settings → Companion → Voice input, default off) is opt-in; with it off the finalized text stays in the bar. Cancel (Escape or the bar's cancel control) discards the utterance; Stop finalizes. Dictation never adds a second dispatch path and never regresses the non-voice bar. Listening is announced on a polite live region (finals only, never partials) and cued by a frozen dot plus a `Listening` chip, never colour or animation alone.
