...
| mission-monitor | ✓ | chat-node (ECE contract) | Delivery-driven agent activity graph (ReactFlow; height-aware chat chain, subagent events excluded) |
...
- **Edge types**: a `calls` edge connects each SubagentNode to its parent ChatNode (`EDGE_STYLES.calls`). The SubagentNode companion column renders LEFT of the chat chain (subagents left, tools right — Spec #2745 final layout; edge handles flipped to source-left/target-right). No Tool→File `reads`/`writes` edges — the non-chat tool/file node classes were removed in Spec #2745.
...
- **Layout toggle (Spec #2752, refined by Spec #2754, reworked by Spec #2756)**: a small floating segmented control (`LayoutModeToggle`, `data-testid="mm-layout-toggle"`) overlays the canvas (`mm-canvas-wrapper`, top-left, outside the SessionTokenBar whose pure-prop contract is untouched) and switches between **Chain** (default, byte-identical deterministic layout above) and **Force**. **Force is a TRUE DISJOINT force-directed layout (Spec #2756 — the Bostock "Force-Directed Graph, Disjoint" pattern, https://observablehq.com/@d3/disjoint-force-directed-graph/2)**: ALL nodes — chat, tools, and subagents — participate in the LIVE animated d3-force simulation (`createLiveForceSimulation` in `lib/layout.ts`, rAF-driven, `freezeOnSettled` whole-sim stop at alpha < alphaMin). The #2754 hybrid is REMOVED: no chat-chain pinning (the `pinned` fx/fy mechanism is gone — chat nodes are force-simulated bodies), no `clampSettledCompanions` halo-band/600px `COMPANION_MAX_PARENT_DISTANCE` clamp, no per-companion chain-y `forceY`. The recipe uses per-node `forceX`/`forceY` POSITIONING forces (one anchor pair per EXCHANGE via `computeExchangeAnchors` — a connected component of the exchange edge set = one chat node + its ToolsNode + its SubagentNodes), NOT `forceCenter`; chat→chat edges are excluded from the sim edge set, so each exchange is its own connected component that drifts into its own blob. Anchors are arranged on a 2-row staggered grid derived from the REAL measured pane (ResizeObserver in `MissionMonitorPanel` → `viewportBounds`; the sim is deferred until the pane is measured, never built against the fixed `VIEWPORT_BOUNDS` fallback), and a bounded containment wall + read clamp keep every settled node within the pane half-extents (the human's "stays inside the viewport" requirement). Synthesized `tools-<anchor> → agent-<anchor>` links make tools cluster with their parent exchange. Structural changes restart the simulation seeded from current positions (new nodes enter at their seed and glide in — no jump); a measured-height reflow no longer re-pins chat (it re-seeds from current positions, preserving settle). Mode switches preserve node data and the viewport (no auto-fit); Force→Chain restore is an instant snap to the deterministic chain. The selected mode persists across panel close/reopen via `usePersistedSetting` under `Fredo_mm_layout_mode` (the shared SQLite-backed settings path — no `localStorage` in feature source). `prefers-reduced-motion` converges the Force layout synchronously (no rAF glide — `snapToSettled` is retained as the reduced-motion a11y path; the pin semantics are gone). Node-type colors are unchanged (no cluster coloring, no swatches/legend); relationship edge strokes use theme tokens (`var(--*)`), not hardcoded hex.
- **Structural disjoint rework (Spec #2758):** Force mode is now Bostock-faithful *structural* disjoint — clusters arise from graph structure alone (disconnected companion-only link components + many-body repulsion + a single centering force), with NO per-exchange/per-node `forceX`/`forceY` anchor positioning forces (`computeExchangeAnchors` from the Spec #2756 recipe is deleted). The sim edge set excludes chat→chat chain links (only chat→tools/subagent links), each exchange is its own connected component that drifts organically into its own blob, and settled nodes stay within the measured pane via bounded containment. Chain mode is byte-identical to the pre-Spec-#2756 baseline (verified by deterministic formula-derived parity tests against the documented closed-form chain constants rather than pixel fixtures).
