...
- **Data source**: `StreamContext.deliveries` (append-only `ContractDelivery[]`) via the `chat-node` ECE contract — `streamFields: ['payload', 'state']`, `transports: ['otlp_grpc']` (as of Spec #615, Mission Monitor subscribes to OTLP gRPC exclusively — Hook and OTLP HTTP transports removed to prevent duplicate nodes from dual-transport events), `eventTypes: ['chat']` (Spec #2688 — `agent_session` removed so session spans can never create chat-node buffers or phantom nodes), `excludePayload: [{ path: 'is_subagent', equals: true }, { path: 'agent.type', equals: 'subagent' }]` (Spec #2723 — subagent events dropped at the engine pre-buffer, so no child-derived delivery reaches IPC/StreamContext/persistence), composite key `(sessionId, correlationId)`, `completeWhen: "state === 'Response'"`. Only the `chat-node` contract is active — `tool-use-lifecycle` and `custom-event` contracts remain deactivated since #593.
- **Graph builder**: `useDeliveryGraph()` — derives ReactFlow nodes/edges from ALL `ContractDelivery` payloads (cross-session, not filtered by sessionId) so the selected session's chat chain is complete. Live deliveries from StreamContext are merged with SQLite-restored deliveries (deduped by delivery.id; restored appended after live for correct incremental cursor processing). Lifecycle mapping: `init` creates nodes, `update` modifies metadata, `end` sets final status (`complete`/`error`). Output is then scoped to the selected session. Since Spec #2723 the adapter injects `is_subagent`/`agent.type` into every span-derived payload of a subagent session and the contract's `excludePayload` drops them at the engine, so **no subagent-derived nodes exist in Mission Monitor** (intentional reversal of Spec #523's SubagentNode rendering — in scope by PO decision, not a regression).
- **Node types**: Agent (Chat) only (as of Spec #2723). SubagentNode rendering and the OTLP/composited subagent detection paths were removed — `SubagentNode.tsx` and the `subagentNode` node type remain registered but are unreachable (spec #523 reversal; no new subagent visualization). Tool and File node creation remains deactivated (since #593); ToolNode/FileNode components and styling remain defined for potential future re-activation.
...
- **Edge types**: `parent` (Agent→Subagent) edges were removed with the Spec #2723 subagent exclusion — the graph now carries no subagent-derived edges. No `calls` (Agent/Subagent→Tool) or `reads`/`writes` (Tool→File) edges — non-chat nodes remain deactivated.
- **Detail Panel**: Slide-in panel on node click, shows type/ID/status/token counts/timestamps/duration. Hides on background click or Escape. Since Spec #2723, Start/End are read from telemetry-derived payload fields (`startTime`/`endTime`, RFC3339 UTC injected by the adapter from the span's `startTimeUnixNano`/`endTimeUnixNano`) with delivery-timestamp fallback for spans lacking an end — displayed via `toLocaleTimeString()` (display format unchanged).
- **Session token bar** (Spec #2717, moved to top by Spec #2723): a compact flex strip at the TOP of the Mission Monitor main view (below the header, above the canvas — never an overlay, `flex-shrink: 0` sibling) shows the **selected session's** total token usage as five abbreviated figures — `In:`/`Ca:`/`Re:`/`Ou:`/`Σ` (full labels in every `aria-label`) — right-aligned, at a visibly smaller height than the #2717 bottom bar (~23px vs ~48px). Total = Input + Cache + Reasoning + Output exactly; Cache = `cacheReadTokens` only, cache write carried but never summed. Hidden when no session is selected. The same five categories render on every chat node as a compact single-line right-aligned row (display-only k-format like `1.2k`/`85k`, full comma values in aria-labels) and in the DetailPanel (five-way labeled rows, unchanged). Session totals are derived from `mergedDeliveries` via `computeSessionTokenTotals()` (per-composite-key last-wins dedupe — Σ per-node == session figure by construction; composited child-session deliveries excluded).
