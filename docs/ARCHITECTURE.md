...
- **Layout toggle (Spec #2752, refined by Spec #2754)**: a small floating segmented control (`LayoutModeToggle`, `data-testid="mm-layout-toggle"`) overlays the canvas (`mm-canvas-wrapper`, top-left, outside the SessionTokenBar whose pure-prop contract is untouched) and switches between **Chain** (default, byte-identical deterministic layout above) and **Force**. **Force is a HYBRID (Spec #2754)**: chat (agent) nodes keep the deterministic chain geometry — EXACTLY the Chain mode's `computeChatChainPositions` output for the same node set (`x = CHAIN_X_CENTER`, measured-height stacking + `CHAIN_GAP`, oldest at top, newest at the bottom; new chat nodes append at the bottom, never mid-chain) — while ONLY the subagent + tools companion nodes participate in the LIVE animated d3-force simulation (`createLiveForceSimulation` in `lib/layout.ts`, rAF-driven, `freezeOnSettled` whole-sim stop at alpha < alphaMin). Chat nodes are `pinned` in the sim (fx/fy-frozen at their chain slots — they stay sim bodies so companions get a forceCollide surface + forceLink endpoint, but are immune to center/forceY drift and never move). The sim is created only when ≥1 companion exists (an all-chat Force graph is deterministic, no rAF). Companions cluster around their PARENT chat node via forceLink (600px distance retained) plus a per-companion `forceY` toward the parent's chain y; a deterministic settled post-pass (`clampSettledCompanions`) enforces the halo-band clamp (companions confined to `x ≤ −294` or `x ≥ 564`, parent-row vertical bounds), the 600px center-to-center parent bound (`COMPANION_MAX_PARENT_DISTANCE`), and zero overlap with the chain/each other (reusing `resolveRectOverlaps`) — settled-only, never per-frame (the glide, AC4, is preserved). Synthesized `tools-<anchor> → agent-<anchor>` links make tools cluster around their parent too. Structural changes restart the simulation seeded from current positions (new companions pre-seeded at their deterministic Chain slots — no jump-to-zero); a measured-height reflow re-pins the chain (chat re-stacks, companions keep positions via the seed). Mode switches preserve node data and the viewport (no auto-fit); Force→Chain restore is an instant snap to the deterministic chain. The selected mode persists across panel close/reopen via `usePersistedSetting` under `Fredo_mm_layout_mode` (the shared SQLite-backed settings path — no `localStorage` in feature source). `prefers-reduced-motion` converges the Force layout synchronously (no rAF glide — `snapToSettled`). Relationship edge strokes use theme tokens (`var(--*)`), not hardcoded hex (Spec #2754 AC5).
