...
**Prerequisite #5 — `OPENCODE_ENABLE_TELEMETRY` env var:** The skill now checks that `OPENCODE_ENABLE_TELEMETRY` is set (any non-empty value) before running tests. This env var enables OTLP gRPC export in the Fredo OpenCode plugin (`config.ts:72`), producing OTLP gRPC transport events alongside the default Hook events.

**⚠️ ECE transport filtering awareness (Spec #638 fix):** `opencode run` produces **Hook transport** events by default — the Mission Monitor's `chat-node` ECE contract specifies `transports: ['otlp_grpc']` (Spec #593/#586) and silently filters out Hook-only events. However, **`OPENCODE_ENABLE_TELEMETRY=1` enables OTLP gRPC export**, so `opencode run` with the env var set produces BOTH Hook AND OTLP gRPC events — matching OTLP-only contracts. ChatNodes WILL appear when the env var is set. All recipe `opencode run` commands are now prefixed with `$env:OPENCODE_ENABLE_TELEMETRY="1";`. Without the env var, only Hook events fire and ChatNodes won't appear. SubagentNodes from background OTLP traffic appear independently. Cross-reference: `fredo emit` bypasses OTLP adapters entirely (IPC → EventBus), so it still does NOT produce OTLP gRPC events. See the skill's ECE Transport Filtering Awareness section for full details.
